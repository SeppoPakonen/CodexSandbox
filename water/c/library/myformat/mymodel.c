...
#define FIRST_LAYER_OFFSET	(NLAYERS_OFFSET  + 4)
	...
	/* create object */
	obj = gfxBeginObject( filename, NULL, ((njoints>0)?(TRUE):(FALSE)) );
	if ( obj == NULL ) {
	...
	for ( i=0; i<njoints; i++ ) {
#ifdef MMF_FIXEDPOINT_API
		pt.x = (fixed)binReadShort( (JOINTS_OFFSET + (i * MMB_SIZEOF_JOINT)) );
		pt.y = (fixed)binReadShort( (JOINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 2) );
		pt.z = (fixed)binReadShort( (JOINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 4) );
		j    = (int)binReadShort( (JOINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 6) );
		k    = binReadInt( (JOINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 8) );
#else
		pt.x = (float)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_JOINT)) );
		pt.y = (float)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 2) );
		pt.z = (float)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 4) );
		j    = (int)binReadShort( (JOINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 6) );
		k    = binReadInt( (JOINTS_OFFSET + (i * MMB_SIZEOF_JOINT) + 8) );
#endif
		gfxAddPivot( obj, pt, j, k );
	}

	/* add point data */
	for ( i=0; i<npoints; i++ ) {
#ifdef MMF_FIXEDPOINT_API
		pt.x = (fixed)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_POINT)) );
		pt.y = (fixed)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_POINT) + 2) );
		pt.z = (fixed)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_POINT) + 4) );
#else
		pt.x = (float)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_POINT)) );
		pt.y = (float)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_POINT) + 2) );
		pt.z = (float)binReadShort( (POINTS_OFFSET + (i * MMB_SIZEOF_POINT) + 4) );
#endif
...
#ifdef MMF_FIXEDPOINT_API
		norm.x = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (i * MMB_SIZEOF_NORMAL)) ) );
		norm.y = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (i * MMB_SIZEOF_NORMAL) + 4) ) );
		norm.z = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (i * MMB_SIZEOF_NORMAL) + 8) ) );
#else
	...
	for ( i=0; i<nlayers; i++ ) {
		...
		for ( j=0; j<ntri; j++ ) {
			...
			quad = (p3 != 0);
#ifdef MMF_FIXEDPOINT_API
			pts[0].x = binReadShort( (POINTS_OFFSET + (p0 * MMB_SIZEOF_POINT)) );
			pts[0].y = binReadShort( (POINTS_OFFSET + (p0 * MMB_SIZEOF_POINT) + 2) );
			pts[0].z = binReadShort( (POINTS_OFFSET + (p0 * MMB_SIZEOF_POINT) + 4) );
			pts[1].x = binReadShort( (POINTS_OFFSET + (p1 * MMB_SIZEOF_POINT)) );
			pts[1].y = binReadShort( (POINTS_OFFSET + (p1 * MMB_SIZEOF_POINT) + 2) );
			pts[1].z = binReadShort( (POINTS_OFFSET + (p1 * MMB_SIZEOF_POINT) + 4) );
			pts[2].x = binReadShort( (POINTS_OFFSET + (p2 * MMB_SIZEOF_POINT)) );
			pts[2].y = binReadShort( (POINTS_OFFSET + (p2 * MMB_SIZEOF_POINT) + 2) );
			pts[2].z = binReadShort( (POINTS_OFFSET + (p2 * MMB_SIZEOF_POINT) + 4) );
			if ( quad ) {
				pts[3].x = binReadShort( (POINTS_OFFSET + (p3 * MMB_SIZEOF_POINT)) );
				pts[3].y = binReadShort( (POINTS_OFFSET + (p3 * MMB_SIZEOF_POINT) + 2) );
				pts[3].z = binReadShort( (POINTS_OFFSET + (p3 * MMB_SIZEOF_POINT) + 4) );
			}
#else
			pts[0].x = (float)binReadShort( (POINTS_OFFSET + (p0 * MMB_SIZEOF_POINT)) );
			pts[0].y = (float)binReadShort( (POINTS_OFFSET + (p0 * MMB_SIZEOF_POINT) + 2) );
			pts[0].z = (float)binReadShort( (POINTS_OFFSET + (p0 * MMB_SIZEOF_POINT) + 4) );
			pts[1].x = (float)binReadShort( (POINTS_OFFSET + (p1 * MMB_SIZEOF_POINT)) );
			pts[1].y = (float)binReadShort( (POINTS_OFFSET + (p1 * MMB_SIZEOF_POINT) + 2) );
			pts[1].z = (float)binReadShort( (POINTS_OFFSET + (p1 * MMB_SIZEOF_POINT) + 4) );
			pts[2].x = (float)binReadShort( (POINTS_OFFSET + (p2 * MMB_SIZEOF_POINT)) );
			pts[2].y = (float)binReadShort( (POINTS_OFFSET + (p2 * MMB_SIZEOF_POINT) + 2) );
			pts[2].z = (float)binReadShort( (POINTS_OFFSET + (p2 * MMB_SIZEOF_POINT) + 4) );
			if ( quad ) {
				pts[3].x = (float)binReadShort( (POINTS_OFFSET + (p3 * MMB_SIZEOF_POINT)) );
				pts[3].y = (float)binReadShort( (POINTS_OFFSET + (p3 * MMB_SIZEOF_POINT) + 2) );
				pts[3].z = (float)binReadShort( (POINTS_OFFSET + (p3 * MMB_SIZEOF_POINT) + 4) );
			}
#endif
			/* get uvee points */
			if ( nuvee ) {
			...
			if ( mirrored ) {
				...
				if ( nnormals ) {
					/* get normals */
#ifdef MMF_FIXEDPOINT_API
					normals[0].x = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p0 * MMB_SIZEOF_NORMAL)) ) );
					normals[0].y = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p0 * MMB_SIZEOF_NORMAL) + 4) ) );
					normals[0].z = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p0 * MMB_SIZEOF_NORMAL) + 8) ) );
					normals[1].x = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p1 * MMB_SIZEOF_NORMAL)) ) );
					normals[1].y = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p1 * MMB_SIZEOF_NORMAL) + 4) ) );
					normals[1].z = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p1 * MMB_SIZEOF_NORMAL) + 8) ) );
					normals[2].x = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p2 * MMB_SIZEOF_NORMAL)) ) );
					normals[2].y = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p2 * MMB_SIZEOF_NORMAL) + 4) ) );
					normals[2].z = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p2 * MMB_SIZEOF_NORMAL) + 8) ) );
					if ( quad ) {
						normals[3].x = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p3 * MMB_SIZEOF_NORMAL)) ) );
						normals[3].y = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p3 * MMB_SIZEOF_NORMAL) + 4) ) );
						normals[3].z = FLOAT_TO_FIXED( binReadFloat( (NORMALS_OFFSET + (p3 * MMB_SIZEOF_NORMAL) + 8) ) );
					}
#else
					normals[0].x = binReadFloat( (NORMALS_OFFSET + (p0 * MMB_SIZEOF_NORMAL)) );
					normals[0].y = binReadFloat( (NORMALS_OFFSET + (p0 * MMB_SIZEOF_NORMAL) + 4) );
					normals[0].z = binReadFloat( (NORMALS_OFFSET + (p0 * MMB_SIZEOF_NORMAL) + 8) );
					normals[1].x = binReadFloat( (NORMALS_OFFSET + (p1 * MMB_SIZEOF_NORMAL)) );
					normals[1].y = binReadFloat( (NORMALS_OFFSET + (p1 * MMB_SIZEOF_NORMAL) + 4) );
					normals[1].z = binReadFloat( (NORMALS_OFFSET + (p1 * MMB_SIZEOF_NORMAL) + 8) );
					normals[2].x = binReadFloat( (NORMALS_OFFSET + (p2 * MMB_SIZEOF_NORMAL)) );
					normals[2].y = binReadFloat( (NORMALS_OFFSET + (p2 * MMB_SIZEOF_NORMAL) + 4) );
					normals[2].z = binReadFloat( (NORMALS_OFFSET + (p2 * MMB_SIZEOF_NORMAL) + 8) );
					if ( quad ) {
						normals[3].x = binReadFloat( (NORMALS_OFFSET + (p3 * MMB_SIZEOF_NORMAL)) );
						normals[3].y = binReadFloat( (NORMALS_OFFSET + (p3 * MMB_SIZEOF_NORMAL) + 4) );
						normals[3].z = binReadFloat( (NORMALS_OFFSET + (p3 * MMB_SIZEOF_NORMAL) + 8) );
					}
#endif
					/* flip around the origin */
					switch( mirroraxis ) {
...
#if 0
		//printf( "%s flip=%d,%d,%d=%d\n", childname, scalex, scaley, scalez, (scalex * scaley * scalez) );
		//if (( scalex * scaley * scalez ) < 0 )
		//	flip = TRUE;
#else
		//flip = FALSE;
		if ( scalex < 0 ) flip = !flip;
		if ( scaley < 0 ) flip = !flip;
		if ( scalez < 0 ) flip = !flip;
#endif
