...
//				mp_work_area = new char[CARD_WORKAREA_SIZE];
				...
				Dbg_MsgAssert( (((size_t)mp_work_area)&0x1f)==0, ("mp_work_area not 32 byte aligned"));
...
int	File::Write( void* data, int len )
{
	...
	Dbg_MsgAssert((((size_t)mp_write_buffer ) & 31 ) == 0, ( "file write buffer must be 32 byte aligned" ));
...
int	File::Read( void* buff, int len )
{
	...
	if (!mp_read_buffer)
	{
		...
		// Ensure the buffer address is 32 byte aligned.
		Dbg_MsgAssert((((size_t)mp_read_buffer ) & 31 ) == 0, ( "file read buffer must be 32 byte aligned" ));
