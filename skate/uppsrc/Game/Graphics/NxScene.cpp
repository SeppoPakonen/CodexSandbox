									   ...
									   CCollStaticTri * &p_coll_sectors, CCollObjTriData * &p_coll_sector_data, Mth::CBBox &bbox, bool is_net)
{
	...
	if (p_base_addr)
	{
...
#ifndef __PLAT_NGC__
		p_base_vert_addr = (uint8 *)(((size_t)(p_base_vert_addr+15)) & 0xFFFFFFF0);	// Align to 128 bit boundary
		...
		p_base_face_addr = (uint8 *)(((size_t)(p_base_face_addr+3)) & 0xFFFFFFFC);	// Align to 32 bit boundary
#else
		...
		p_base_face_addr = (uint8 *)(((size_t)(p_base_face_addr+15)) & 0xFFFFFFF0);	// Align to 128 bit boundary
...
void			CScene::UpdateSuperSectors(Lst::Head<CSector> *p_additional_remove_list)
{
	...
	/*
	Mem::Heap *p_top_heap = MemManagerHandleTopDownHeap();
