...
//		if( p_Ngc_details->m_pass.m_texture.p_data == p_texture )
	...
	p_splat_details_table->PutItem((size_t)p_Ngc_details, p_Ngc_details );
...
/*                                                                */
/******************************************************************/
inline bool line_segment_intersection( float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4 )
{
...
/*                                                                */
/******************************************************************/
inline bool tri_texture_intersect( float u0, float v0, float u1, float v1, float u2, float v2 )
{
...
void plat_texture_splat_cleanup( void )
{
	...
	while( p_details )
	{
		...
		p_splat_details_table->FlushItem((size_t)p_Ngc_details );
...
//.col
							...
							Dbg_Assert((size_t)p_array - (size_t)p_array_loop < ( sizeof(sNgcVert) * 1024 ));
				...
				//			int vert_idx = 0;
							...
							{
								Dbg_Assert(((size_t) p_array_loop) < ((size_t) p_array));
...
void plat_shatter_cleanup( void )
{
	...
	while( p_details )
	{
		...
		p_shatter_details_table->FlushItem((size_t)p_Ngc_details );
		delete p_Ngc_details;
	}
}
...
void plat_shatter( CGeom *p_geom )
{
	...
	for( uint32 m = 0; m < p_Ngc_geom->m_num_mesh; ++m )
	{
		...
		if( num_indices >= 3 )
		{
			...
			p_shatter_details_table->PutItem((size_t)p_details, p_details );
...
Lst::HashTable< sShatterInstanceDetails >		*p_shatter_details_table;
sTriSubdivideStack								triSubdivideStack;
Mth::Vector										shatterVelocity;
float											shatterAreaTest;
float											shatterVelocityVariance;
float											shatterSpreadFactor;
float											shatterLifetime;
float											shatterBounce;
