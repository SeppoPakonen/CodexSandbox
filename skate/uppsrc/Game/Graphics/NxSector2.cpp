...
//#ifdef SHORT_VERT
//		p_vertex_positions = new (MemManagerHandleTopDownHeap()) float[num_vertices * 3];
//#else
...
//
//		sbytes = ( bytes + 3 ) / 4;
//		p_double = new /*(MemManagerHandleTopDownHeap())*/ uint32[dbytes + sbytes];
...
//
//		float * p_temp_tex_coords = new (MemManagerHandleTopDownHeap()) float[num_vertices * 2 * num_tc_sets];
...
//		p_temp_uv_remap = num_tc_sets ? new (MemManagerHandleTopDownHeap()) uint16[num_vertices*num_tc_sets] : NULL;
//
...
//			float * p_unique_tex_coords = new (MemManagerHandleTopDownHeap()) float[num_vertices * 2 * num_tc_sets];
//
...
//		p_temp_col_remap = p_vertex_colors ? new (MemManagerHandleTopDownHeap()) uint16[num_vertices] : NULL;
//
//		for ( int c = 0; c < num_vertices; c++ )
//		{
...
//		if ( reduce )
//		{
//			uint32 * p_temp_colors = new (MemManagerHandleTopDownHeap()) uint32[num_vertices];
//			memcpy( p_temp_colors, p_vertex_colors, num_vertices * 4 );
//			delete [] p_vertex_colors;
//			uint32* p_unique_colors = new (MemManagerHandleTopDownHeap()) uint32[num_vertices];
//
//			// Now, squeeze down to only unique uvs.
//			int numUnique = 0;
//			for ( int lp = 0; lp < num_vertices; lp++ )
...
//
//					// Construct the DL.
//					MemManagerHandleTopDownHeap()->PushAlign( 32 );
//#define DL_BUILD_SIZE (4*1024)
//					uint8 * p_build_dl = new (MemManagerHandleTopDownHeap()) uint8[DL_BUILD_SIZE];
...
//					MemManagerHandleTopDownHeap()->PopAlign();
//
//
//
//
