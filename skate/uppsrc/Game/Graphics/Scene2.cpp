		...
		if( ( p_material1->m_flags & (1<<0) ) == ( p_material2->m_flags & (1<<0) ) )
		{
			...
			else
			{
				...
				if( p_material1->m_base_blend == p_material2->m_base_blend )
				{
					...
					{
						return ((size_t)p_material1 > (size_t)p_material2 ) ? 1 : -1;
					}
					else
					{
						return ((size_t)p_material1->m_texture_dl_id < (size_t)p_material2->m_texture_dl_id ) ? 1 : -1;
					}
				}
				else
				{
					return ((size_t)p_material1->m_base_blend > (size_t)p_material2->m_base_blend ) ? 1 : -1;
				}
			}
		}
...
void MaterialSubmit( sMesh * p_mesh, sScene *pScene )
{
	...
	for ( int lp = 0; lp < p_mat->m_passes; lp++, p_pass++ )
	{
		if ( p_pass->m_flags & ( (1<<1) | (1<<2) ) )
		{
			...
			if( p_pass->m_flags & (1<<1) )
			{
				...
				MTXInvXpose(EngineGlobals.local_to_camera.GetMtx(), mv );
...
//						   EngineGlobals.local_to_camera.GetMtx(),
//						   p, vp, &rx, &ry, &rz );
