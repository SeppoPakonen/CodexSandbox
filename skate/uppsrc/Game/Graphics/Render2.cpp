...
void set_blend_mode( uint64 mode )
{
	...
	uint32 actual_mode	= (size_t)mode;
//	uint32 fixed_alpha	= (size_t)( mode >> 32 );
...
/*                                                                */
/******************************************************************/
static int cmp_render( const void *p1, const void *p2 )
{
...
//	p_scene->m_flags		|= SCENE_FLAG_RECEIVE_SHADOWS;
	...
	pTextureProjectionDetailsTable->PutItem((size_t)p_texture, p_details );
}
...
{
	sTextureProjectionDetails *p_details = pTextureProjectionDetailsTable->GetItem((size_t)p_texture );
	if( p_details )
	{
		pTextureProjectionDetailsTable->FlushItem((size_t)p_texture );
		delete p_details;
	}
}
...
//	MTXConcat( light, EngineGlobals.shadow_camera.GetMtx(), light );
//#ifndef SHORT_VERT
...
//		GX::SetZMode( GX_TRUE, GX_LEQUAL, GX_TRUE );
		...
		qsort( sortedMeshArray, next_sorted_mesh_entry, sizeof( sSortedMeshEntry ), cmp_render );
...
//				if ( (int)p_mesh->mp_dl->m_material.p_header->m_layer_id == g_kill_layer ) return;
				...
				{
					// Deal with verts outside of 65535.
					if ( p_mesh->mp_dl->m_array_base != (size_t)array_base )
					{
