...
static 	Tmr::Time	stats_s_connect_start_time = 0;
...
void	StatsMan::s_stats_logic_code( const Tsk::Task< StatsMan >& task )
{
	...
	switch( man.m_state )
	{
		...
		case vSTATS_STATE_CONNECTING:
			if(( Tmr::GetTime() - stats_s_connect_start_time ) > vSTATS_CONNECT_TIMEOUT )
			{
...
void	StatsMan::Connect( void )
{
	...
	stats_s_connect_start_time = Tmr::GetTime();

	Script::RunScript( "stats_log_in_profile" );

    net_thread_data.m_pEntry = (void*)&s_threaded_stats_connect;
...
bool	StatsMan::ScriptReportStats(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	final_ = pParams->ContainsFlag( CRCD(0x132aa339,"final_"));
	stats_man = gamenet_man->mpStatsMan;
	stats_man->ReportStats( final_ );
...
void 	StatsMan::ReportStats( bool final_ )
{
    ...
    result = SendGameSnapShot( m_cur_game, NULL, (int) final_ );
