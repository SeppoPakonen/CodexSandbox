...
#	if !defined( __PLAT_NGC__ ) || ( defined( __PLAT_NGC__ ) && !defined( __NOPT_FINAL__ ) )
	uint codesize = ((size_t)(_code_end) - (size_t)(_code_start))/1024;
	uint datasize = ((size_t)(_data_end) - (size_t)(_code_end))/1024;
...
void*	Malloc( size_t size )
{
	...
	void *v =  MemManagerHandleNew( size, true );
...
int		Available()
{
	return MemManagerHandleAvailable();
}

void* ReallocateDown( size_t newSize, void *pOld )
{
	return MemManagerHandleReallocateDown(newSize,pOld);
}

void* ReallocateUp( size_t newSize, void *pOld )
{
	return MemManagerHandleReallocateUp(newSize,pOld);
}

void* ReallocateShrink( size_t newSize, void *pOld )
{
	return MemManagerHandleReallocateShrink(newSize,pOld);
}
...
{
	return MemManagerHandleValid( pAddr );
}
...
{
	return MemManagerHandleGetAllocSize( pAddr );
}
...
void*	Realloc( void* mem, size_t newSize )
{
	...
	if ( newSize )
	{
//		MemManagerHandlePushContext(Manager::sHandle().TopDownHeap());
		ptr = MemManagerHandleNew( newSize, true );
...
void*	Calloc( size_t numObj, size_t sizeObj )
{
	...
	return MemManagerHandleNew(( numObj * sizeObj ), true );
}
...
void	PushMemProfile(char *p_type)
{
	if( s_active )
	{
		...
		if (!s_profile_list)
		{
			if (Config::GotExtraMemory())
			{
				MemManagerHandlePushContext(Manager::sHandle().DebugHeap());
...
void	AllocMemProfile(Allocator::BlockHeader* p_block)
{
	if( s_active )
	{
		if (sp_current_profile)
		{
			...
			if (MemManagerHandleGetContextAllocator() != MemManagerHandleDebugHeap())
			{
