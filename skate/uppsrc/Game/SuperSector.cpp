		...
		for( int j = 0; j < NUM_PARTITIONS_Z; j++ )
		{
			...
			if (num_change < 0)				// Re-alloc array down
			{
				if (!p_super_sector->m_CollSectorList ||
					!MemManagerHandleReallocateShrink(sizeof(Nx::CCollStatic *) * new_size, p_super_sector->m_CollSectorList))
				{
			...
			{
				if (!p_super_sector->m_CollSectorList ||
				    !MemManagerHandleReallocateUp(sizeof(Nx::CCollStatic *) * new_size, p_super_sector->m_CollSectorList))
				{
