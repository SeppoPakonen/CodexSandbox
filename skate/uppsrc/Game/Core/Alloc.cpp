...
#else
#ifdef __PLAT_NGC__
const uint	Allocator::BlockHeader::sSize = (size_t)nAlignUpBy( sizeof(BlockHeader), 5 );
#else
const uint	Allocator::BlockHeader::sSize = (size_t)nAlignUpBy( sizeof(BlockHeader), 4 );
#endif		// __PLAT_NGC__
#endif
...
void	Allocator::s_set_id( void* pAddr )
{
	...
	BlockHeader* p_bheader = (BlockHeader*)( ((size_t)pAddr) - BlockHeader::sSize );
#ifdef __EFFICIENT__
	p_bheader = (BlockHeader*)(((size_t)p_bheader) - p_bheader->mPadBytes);
#endif		// __EFFICIENT__
...
{
	BlockHeader* p_ret = (BlockHeader*)(((size_t)pAddr) - BlockHeader::sSize);
#ifdef __EFFICIENT__
	p_ret = (BlockHeader*)(((size_t)p_ret) - p_ret->mPadBytes);
#endif		// __EFFICIENT__
...
void 	Allocator::dump_used_list( void )
{
	...
	while ( p_header )
	{
		...
		Dbg_Message ( "%p  %x   (%p)", (void*)((size_t)p_header + BlockHeader::sSize ),
...
void * Allocator::find_block(void *p)
{
	...
	while ( p_header )
	{
		...
		void * p_start = (void*)((size_t)p_header + BlockHeader::sSize);
		int size = p_header->mSize;
		void * p_end = (void*)((size_t)p_start + size);
