...
inline Mem::Allocator::BlockHeader*	Heap::next_addr( Allocator::BlockHeader* pHeader )
{
	...
	return (BlockHeader*)( (size_t)pHeader + BlockHeader::sSize + pHeader->mSize );
}
...
{
#ifdef __EFFICIENT__
	memset( (void*)((size_t)pBlock + Mem::Allocator::BlockHeader::sSize ), vTRASH_ALLOCATED_BLOCK & 0xff, pBlock->mSize );
#else
	uint64* ptr = (uint64*)((size_t)pBlock + Mem::Allocator::BlockHeader::sSize );
...
{
#ifdef __EFFICIENT__
	memset( (void*)((size_t)pBlock + Mem::Allocator::BlockHeader::sSize ), vTRASH_FREE_BLOCK & 0xff, pBlock->mSize );
#else
	uint64* ptr = (uint64*)((size_t)pBlock + Mem::Allocator::BlockHeader::sSize );
...
{
#ifdef __EFFICIENT__
	memset( (void*)((size_t)pBlock), vTRASH_FREE_BLOCK & 0xff, Mem::Allocator::BlockHeader::sSize );
#else
	uint64* ptr = (uint64*)((size_t)pBlock);
...
void	Heap::free ( BlockHeader* pFreeBlock )
{
	...
	if ( ((size_t)pFreeBlock + BlockHeader::sSize) == REPORT_ON)
	{
...
// when memory is fragmented (which it often is, in the middle of things)
	...
	// block
	while ( p_after && ( (size_t)p_after < (size_t)pFreeBlock ))
	{
		p_2before = p_before;
		p_before = p_after;
	...
	if ( ( m_dir == vBOTTOM_UP ) && ( !p_after ) &&
		 ( mp_top == (void*)((size_t)pFreeBlock + pFreeBlock->mSize + BlockHeader::sSize )))
	{
		if( p_before )
		{
		...
		mp_top = (void*)((size_t)mp_top - ( pFreeBlock->mSize + BlockHeader::sSize) * m_dir );
		pFreeBlock->~BlockHeader();
		mFreeBlocks--;
		mFreeMem -= pFreeBlock->mSize;
		mUsedMem -= BlockHeader::sSize;
...
//	{
//		printf ("\nAllocating %d\n",size);
//		printf ("%s\n",MemManagerHandleGetContextName());
//	}
	#endif
...
//	if (size == 8688) while (1);
	...
	int offset_bytes;
	size = (size_t)nAlignUpBy( size, 2 );	// all allocations aligned by 4 bytes
#else
#ifdef __PLAT_NGC__
	size = (size_t)nAlignUpBy( size, 5 );	// all allocations aligned by 16 bytes
#else
	size = (size_t)nAlignUpBy( size, 4 );	// all allocations aligned by 16 bytes
#endif
#endif
...
#ifdef __EFFICIENT__
		int off = ( ( ( (size_t)p_header + BlockHeader::sSize ) + ( align - 1 ) ) & -align ) - ( (size_t)p_header + BlockHeader::sSize );
	...
	{
#ifdef __EFFICIENT__
		offset_bytes = ( ( ( (size_t)p_freeblock + BlockHeader::sSize ) + ( align - 1 ) ) & -align ) - ( (size_t)p_freeblock + BlockHeader::sSize );
#endif
		if ( p_prev )
		{
...
#ifdef __EFFICIENT__
		BlockHeader*	p_leftover 	= (BlockHeader*)((size_t)p_freeblock + BlockHeader::sSize + size + offset_bytes );
		int				new_size 	= p_freeblock->mSize - size - BlockHeader::sSize - offset_bytes;
#else
		BlockHeader*	p_leftover 	= (BlockHeader*)((size_t)p_freeblock + BlockHeader::sSize + size );
	...
	else				// request extra space from region
	{
		...
		{
			offset_bytes = ( (size_t)p_freeblock + BlockHeader::sSize ) - ( ( (size_t)p_freeblock + BlockHeader::sSize ) & -align );
	...
	if ( p_freeblock )
	{
		...
		if ( ((size_t)p_freeblock + BlockHeader::sSize) == REPORT_ON)
		{
...
#ifdef __EFFICIENT__
		check_address( (void*)((size_t)p_freeblock + BlockHeader::sSize + offset_bytes), size );

		// Fill padding bytes with bytes offset value.
		p_freeblock->mPadBytes = offset_bytes;
		uint8 * p8 = (uint8*)((size_t)p_freeblock + BlockHeader::sSize);
		for ( int lp = 0; lp < offset_bytes; lp++ )
		{
			*p8++ = offset_bytes;
		}

		return (void*)((size_t)p_freeblock + BlockHeader::sSize + offset_bytes);
#else
		check_address( (void*)((size_t)p_freeblock + BlockHeader::sSize), size );

		return (void*)((size_t)p_freeblock + BlockHeader::sSize);
#endif		// __EFFICIENT__
	}
...
//	Mem::Heap* heap = MemManagerHandleBottomUpHeap();
...
void* Heap::reallocate_down( size_t newSize, void *pOld )
{
...
#ifdef __EFFICIENT__
	newSize = (size_t)nAlignUpBy( newSize, 2 );	// all allocations aligned by 4 bytes
#else
#ifdef __PLAT_NGC__
	newSize = (size_t)nAlignUpBy( newSize, 5 );	// all allocations aligned by 32 bytes
#else
	newSize = (size_t)nAlignUpBy( newSize, 4 );	// all allocations aligned by 16 bytes
#endif
#endif		// __EFFICIENT__
	...
	Dbg_MsgAssert( (BlockHeader*)(((size_t)p_new)+p_new_block->mSize) == p_old_block,("reallocate_down failed! New block is not directly below the old."));
...
#ifdef __EFFICIENT__
	return (void*)((size_t)p_new_block + BlockHeader::sSize + p_new_block->mPadBytes);
#else
	return (void*)((size_t)p_new_block + BlockHeader::sSize);
#endif		// __EFFICIENT__
}
...
void *Heap::reallocate_up( size_t newSize, void *pOld )
{
	...
	BlockHeader* p_new_block=BlockHeader::sRead( p_new );
	if ( (BlockHeader*)(((size_t)pOld)+p_old_block->mSize) != p_new_block)
	{
		// It isn't!
		free(p_new_block);
		return NULL;
...
void *Heap::reallocate_shrink( size_t newSize, void *pOld )
{
	...
	BlockHeader *p_new_block=(BlockHeader*)((size_t)pOld+p_old_block->mSize-size_diff);

	#ifdef	__PLAT_NGPS__
	Dbg_MsgAssert(((size_t)p_new_block&0xf) == 0,("p_new_block odd (%p), pOld = %p, p_old_block = %p, p_old_block->mSize = %d, size_diff = %d",
