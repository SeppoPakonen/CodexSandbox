...
void Pool::dump_free_list( void )
{
	...
	while ( p_header )
	{
		...
		Dbg_Message ( "%p  %d   (%p)", (void*)((size_t)p_header + sizeof( BlockHeader )),
...
void*	Pool::allocate( size_t size, bool assert_on_fail )
{
	...
	if ( mp_free_list )
	{
		...
		return (void*)((size_t)p_header + BlockHeader::sSize);
	}
...
: Allocator( region, dir, "Pool" ), m_count( count), m_size( size )
{
	...
	for ( uint i = 0; i < ( m_count - 1 ); i++ )
	{
		...
		BlockHeader* p_nextblock = (BlockHeader*)( (size_t)p_freeblock + BlockHeader::sSize + m_size );
