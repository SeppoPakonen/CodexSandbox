...
//	printf ("p_preContained = %p\n, total_data = %d, name = (%p) %s",p_preContained,total_data_size,p_preContained->mpName,p_preContained->mpName);
	...
	uint32 p_next=(size_t)p_preContained->mpName;
...
void LoadPre(const char *p_preFileName)
{
	...
	for (uint32 f=0; f<num_files; ++f)
	{
		...
		new_pre_buffer_size+=(size_t)p_contained->mpName-(size_t)p_contained;
	...
	if (MemManagerHandleGetContextDirection()==Mem::Allocator::vTOP_DOWN)
	{
	...
	for (uint32 f=0; f<num_files; ++f)
	{
		...
		p_dest=(uint8*)( ((size_t)p_dest+15)&~15 );
	...
	p_dest_header->mSize=(size_t)p_dest_contained-(size_t)p_dest_header;

	//printf("Wasted space = %d\n",new_pre_buffer_size-((size_t)p_dest_contained-(size_t)p_new_file_data));
...
void* Load(const char* p_fileName)
{
	...
	if (p_contained_file)
	{
		...
		uint32 p_data=(size_t)p_contained_file->mpName+p_contained_file->mNameSize;
		p_data=(p_data+15)&~15;
		return (void*)p_data;
	}
