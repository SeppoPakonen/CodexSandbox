											...
											void *p_base_face_addr, void *p_base_node_addr, void *p_base_face_idx_addr)
{
	...
	mp_intensity = (unsigned char *) ((size_t) p_base_vert_addr + (size_t)mp_intensity);
	mp_cloned_vert_pos = NULL;
#else
#ifdef FIXED_POINT_VERTICES
	// Get indexes
	int start_vert_pos_offset = (size_t) mp_float_vert;
	int start_intensity_index = (size_t) mp_intensity;

	// Find first element
	mp_float_vert = (SFloatVert *) ((size_t) p_base_vert_addr + start_vert_pos_offset);
	mp_intensity = (uint8 *) p_base_intensity_addr;
	mp_intensity += start_intensity_index;
#else
	// Get indexes
	int start_vert_pos_offset = (size_t) mp_vert_pos;

	// Find first element
	mp_vert_pos = (Mth::Vector *) ((size_t) p_base_vert_addr + start_vert_pos_offset);

	mp_intensity = NULL;
#endif // FIXED_POINT_VERTICES
#endif		// __PLAT_NGC__

	mp_faces = (SFace *)((size_t) mp_faces + (size_t) p_base_face_addr);
	...
	// Init BSP tree
	mp_bsp_tree = (CCollBSPNode *)((size_t) mp_bsp_tree + (size_t) p_base_node_addr);
...
CCollBSPNode * CCollBSPNode::clone(bool instance)
{
	...
	if (num_nodes > 0)
	{
		...
		int node_address_diff = (size_t) p_new_bsp_array - (size_t) this;
		int face_address_diff = (size_t) p_new_face_index_array - (size_t) p_orig_face_index_array;
		for (int i = 0; i < num_nodes; i++)
		{
			if (p_new_bsp_array[i].IsNode())
			{
				// Adjust branch pointers
				p_new_bsp_array[i].m_node.m_children.SetBasePointer((CCollBSPNode *)((size_t) p_new_bsp_array[i].m_node.m_children.GetBasePointer() + node_address_diff));
			}
			else
			{
				// Adjust face index pointer
				p_new_bsp_array[i].m_leaf.mp_face_idx_array = (FaceIndex *) ((size_t) p_new_bsp_array[i].m_leaf.mp_face_idx_array + face_address_diff);
			}
		}
...
bool	CCollObjTriData::s_init_tree(CCollBSPNode *p_tree, void *p_base_node_addr, void *p_base_face_idx_addr)
{
	if (p_tree->IsLeaf())
	{
		...
		int face_idx = (size_t) p_tree->m_leaf.mp_face_idx_array;
		p_tree->m_leaf.mp_face_idx_array = (FaceIndex *) p_base_face_idx_addr;
		p_tree->m_leaf.mp_face_idx_array += face_idx;

		//Dbg_Assert(((size_t) p_leaf->mp_less_branch) == -1);
		//Dbg_Assert(((size_t) p_leaf->mp_greater_branch) == -1);
		...
		// Set branch pointers
		p_tree->m_node.m_children.SetBasePointer((CCollBSPNode *)((size_t) p_tree->m_node.m_children.GetBasePointer() + (size_t) p_base_node_addr));
