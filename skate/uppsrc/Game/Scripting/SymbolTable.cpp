...
static CSymbolTableEntry *st_sp_hash_table=NULL;

void CreateSymbolHashTable()
{
	Dbg_MsgAssert(st_sp_hash_table==NULL,("st_sp_hash_table not NULL ?"));
	st_sp_hash_table=new CSymbolTableEntry[1<<NUM_HASH_BITS];
}

void DestroySymbolHashTable()
{
	Dbg_MsgAssert(st_sp_hash_table!=NULL,("st_sp_hash_table is NULL ?"));
	delete[] st_sp_hash_table;
	st_sp_hash_table=NULL;
}
...
CSymbolTableEntry *LookUpSymbol(uint32 checksum)
{
	...
	*/
	...
	Dbg_MsgAssert( st_sp_hash_table, ( "No symbol table...  was CreateSymbolHashTable() called?" ) );
#endif

	// Instead of asserting if st_sp_hash_table is NULL, just return NULL.
...
//	}
    ...
    CSymbolTableEntry *p_sym=&st_sp_hash_table[ checksum & ((1<<NUM_HASH_BITS)-1) ];
...
void RemoveSymbol(CSymbolTableEntry *p_sym)
{
    ...
    CSymbolTableEntry *p_head=&st_sp_hash_table[ p_sym->mNameChecksum & ((1<<NUM_HASH_BITS)-1) ];
...
CSymbolTableEntry *GetNextSymbolTableEntry(CSymbolTableEntry *p_sym)
{
	...
	if (!p_sym)
	{
		...
		++s_current_hash_index;
		p_sym=st_sp_hash_table+s_current_hash_index;
