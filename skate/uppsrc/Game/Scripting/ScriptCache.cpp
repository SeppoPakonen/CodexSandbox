...
void CScriptCache::remove_some_old_scripts(int space_required, uint32 scriptName)
{
	while (true)
	{
		int largest_malloc_possible=MemManagerHandleScriptHeap()->LargestFreeBlock();
		...
		//printf("%d: Deleting %s from cache, new free space = %d\n",space_required,Script::FindChecksumName(mp_last_zero_usage->mScriptNameChecksum),MemManagerHandleScriptHeap()->LargestFreeBlock()-128);
		delete_entry(mp_last_zero_usage);
	}
}
