...
{
	int len = strlen(p);
	if ((size_t)(p_pos + len) > (size_t)(p_buffer + DUMP_LEN-16))
	{
		dump_flush();
	}
...
bool ScriptGetElement(Script::CStruct *pParams, Script::CScript *pScript)
{
	...
	if (p_array)
	{
		...
		case ESYMBOLTYPE_STRUCTURE:
		{
			...
			pScript->GetParams()->AddComponent(0xbebfa1c6/*Element*/,ESYMBOLTYPE_STRUCTUREPOINTER,(size_t)p_new);
			break;
		}
		case ESYMBOLTYPE_ARRAY:
...
#ifdef __NOPT_ASSERT__
	Mem::Allocator* pCurrentHeapContext = MemManagerHandleGetContextAllocator();
	if ( !( pCurrentHeapContext == MemManagerHandleCutsceneBottomUpHeap()
		 || pCurrentHeapContext == MemManagerHandleBottomUpHeap() ) )
	{
...
// ********************************************************************************************
	...
	else
	{
		...
		MemManagerHandleDeleteSkaterHeaps();
	}
	...
	if( skate_mod->ShouldAllocateNetMiscHeap())
	{
		...
		MemManagerHandleInitNetMiscHeap();
	}
	else
	{
		MemManagerHandleDeleteNetMiscHeap();
	}
...
#	ifdef __PLAT_NGPS__
	...
	{
		MemManagerHandleInitInternetHeap();
	}
	else
	{
		MemManagerHandleDeleteInternetHeap();
	}
#	endif // __PLAT_NGPS__
...
//	MemViewToggle();
	...
	else
	{
		...
		MemManagerHandlePushContext( pHeap );
	}
