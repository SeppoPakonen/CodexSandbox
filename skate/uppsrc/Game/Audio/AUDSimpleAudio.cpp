...
BOOL AUDSimpleInitAudioDecoder(void)
	...
	for(i=1; i<AUD_AUDIO_NUMAIBUFFERS; i++)
		audioPlayBuffer[i] = (void *)((size_t)audioPlayBuffer[i - 1] + (2 * sizeof(s16) * AUD_AUDIO_AIBUFFERSAMPLES));
...
static BOOL audioDecode(u32 numChannels, const s16 **samples, u32 sampleNum, void* _UNUSED(userData))
	...
	if (audioReadBufferWritePos >= audioReadBufferReadPos)
	{
		...
		if ((len1 = (audioReadBufferNumSamples - audioReadBufferWritePos)) >= sampleNum)
		{
			...
			writeChannelData((s16 *)((size_t)audioReadBuffer + audioReadBufferWritePos * sampleSize),numChannels,samples,0,sampleNum);
		}
		else
		{
			// Dual buffer case
			writeChannelData((s16 *)((size_t)audioReadBuffer + audioReadBufferWritePos * sampleSize),numChannels,samples,0,len1);
...
static void audioCopy(u32 smpOffset,u32 numSamples)
	...
	destAddrL = (s16 *)((size_t)audioPlayBuffer[audioPlayBufferWriteIndex] + smpOffset * sizeof(s16));
	destAddrR = (s16 *)((size_t)audioPlayBuffer[audioPlayBufferWriteIndex] + smpOffset * sizeof(s16) + (AUD_AUDIO_AIBUFFERSAMPLES * sizeof(s16)));

	// Single or dual channel setup?
    if (audioReadBufferNumChannels == 2)
		{
		// Stereo! Get source address of data...
		srcAddr = (s16 *)((size_t)audioReadBuffer + audioReadBufferReadPos * 2 * sizeof(s16));
		...
		// Get source address...
		srcAddr = (s16 *)((size_t)audioReadBuffer + audioReadBufferReadPos * sizeof(s16));
...
//			OSReport( "AX audio buffer underrun!\n" );
		...
		if( audioPlayBufferEnabled )
		{
			...
			if( numUpdate != 0 )
			{
				...
				for( i = 0; i < numUpdate; i++ )
				{
					...
					leftSource	= (size_t)audioPlayBuffer[audioPlayBufferWriteIndex];
					rightSource	= (size_t)audioPlayBuffer[audioPlayBufferWriteIndex] + ( AUD_AUDIO_AIBUFFERSAMPLES * sizeof( s16 ));
		...
		else
		{
			...
			for( i = 0; i < audioPlayBufferNeededAudioFrames; i++ )
			{
				...
				leftSource = (size_t)audioPlayBuffer[audioPlayBufferWriteIndex];
				rightSource = (size_t)audioPlayBuffer[audioPlayBufferWriteIndex] + (AUD_AUDIO_AIBUFFERSAMPLES * sizeof(s16)) / 2;
				leftTarget = 2 * (AX_ARAM_LEFT_CHANNEL + audioPlayBufferWriteIndex * AUD_AUDIO_AIBUFFERSAMPLES);
				rightTarget = 2 * (AX_ARAM_RIGHT_CHANNEL + audioPlayBufferWriteIndex * AUD_AUDIO_AIBUFFERSAMPLES);
