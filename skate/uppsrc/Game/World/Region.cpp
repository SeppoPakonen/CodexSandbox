...
void	Region::init( void* pStart, void* pEnd )
{
	...
	m_min_free  = (size_t)pEnd - (size_t)pStart;
	...
	if (!Config::CD())
	{
		...
		uint 	size = (size_t)mp_end - (size_t)mp_start;
...
void*		Region::Allocate( Allocator* pAlloc, size_t size, bool assert_on_fail )
{
	...
	void* p_top = (void*) (( (size_t)pAlloc->mp_top ) + ( size * pAlloc->m_dir ) );
...
AllocRegion::AllocRegion( size_t size )
{
	...
	init( mp_start, (void*)( (size_t)mp_start + size ));
}
...
int Region::MemAvailable( void )
{
	...
	int bot = (size_t)((m_alloc[0]) ? m_alloc[0]->mp_top : mp_start);
	int top = (size_t)((m_alloc[1]) ? m_alloc[1]->mp_top : mp_end);
