...
#	endif
	...
	int num_indices=sizeof(p_end_verts_a)/sizeof(size_t);
...
#	endif
#	endif
	int num_bottom_vertices=sizeof(p_bottom_vert_indices)/sizeof(size_t);
...
void CEditedRail::GetDebugInfo( Script::CStruct* p_info )
{
	...
	if (num_points)
	{
		...
		while (p_point)
		{
			...
			p_struct->AddInteger(CRCD(0x41a51a93,"mpClonedRailSector"),(size_t)p_point->mpClonedRailSector);
			p_struct->AddInteger(CRCD(0x79ffd768,"mpPostSector"),(size_t)p_point->mpPostSector);
...
void CRailEditorComponent::ReadFromStructure(Script::CStruct *p_info)
{
	...
	Dbg_MsgAssert((size_t)p_rails->GetSize() <= GetNumFreeRails(),("Too many rails, got %d, max is %d",p_rails->GetSize(),GetNumFreeRails()));
