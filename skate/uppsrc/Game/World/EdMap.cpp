...
void CMetaPiece::SetRot(Mth::ERot90 newRot)
{
	...
	if (mp_additional_desc_tab)
	{
		MemManagerHandlePushContext(CParkManager::sInstance()->GetGenerator()->GetParkEditorHeap());
...
{
	Dbg_Assert(this);
	Dbg_MsgAssert(((size_t)this) != 0x03030303,("Bad CMapListNode pointer ! (0x03030303)"));
	Dbg_MsgAssert(((size_t)this) != 0x55555555,("Bad CMapListNode pointer ! (0x55555555)"));
	Dbg_Assert(mp_meta);
	return mp_meta;
}
...
CConcreteMetaPiece *CParkManager::CreateConcreteMeta(CAbstractMetaPiece *pSource, bool isSoft)
{
	...
	MemManagerHandlePushContext(mp_generator->GetParkEditorHeap());
...
bool CParkManager::EnoughMemoryToResize(GridDims new_dims)
{
	...
	Mem::Heap *p_top_down_heap = MemManagerHandleTopDownHeap();
...
void CParkManager::write_compressed_map_buffer()
{
	...
	int buffer_used_size = (size_t) p_meta_entry - (size_t) p_header;
	...
	buffer_used_size = (size_t) p_out_gap - (size_t) p_header;
	Dbg_MsgAssert(buffer_used_size <= COMPRESSED_MAP_SIZE, ("compressed map buffer (%d) needs to be bigger than (%d)",COMPRESSED_MAP_SIZE,buffer_used_size));
