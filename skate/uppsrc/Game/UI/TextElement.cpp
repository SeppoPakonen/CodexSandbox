...
void CTextElement::SetText(const char *pText)
{
	...
	if (!mp_text || (size_t)(strlen(mp_text)) < new_length-1)		  // note, does not account for actual allocated size, but will work is string are same size
	{
		if (mp_text)
			delete mp_text;
...
void CTextBlockElement::SetFont(uint32 font_checksum)
{
	...
	m_num_visible_lines = (size_t) m_base_h / (size_t) (word_h * m_internal_scale);
	m_object_flags |= CScreenElement::vCHANGED_STATIC_PROPS;
}
...
//	char parsed_lines[MAX_LINES][MAX_CHARS];
	...
	if ( m_virtual_out_line >= m_num_visible_lines )
	{
		...
		if ( m_allow_expansion )
		{
			...
			m_num_visible_lines = (size_t) m_base_h / (size_t) (word_h * m_internal_scale * m_line_spacing_scale );
...
CTextBlockElement::EParseResult CTextBlockElement::parse_next_word(char *pWordBuf, const char **ppSource)
{
	...
	while (keep_scanning)
	{
		...
		Dbg_Assert((size_t) p_out - (size_t) pWordBuf < MAX_CHARS - 1);
	}
...
bool CTextBlockElement::Concatenate( const char* pText, bool enforce_max_width, bool last_line )
{
	...
	int length = GetLength();
	Dbg_MsgAssert( length < (size_t)( MAX_EDITABLE_TEXT_BLOCK_LENGTH - strlen( pText ) ), ( "TextBlock too long to concatenate" ) );
#endif
