...
int	handle_ack( MsgHandlerContext *context )
{
		...
		( send_time > 0 ))
	{
		...
		if( latency_value > (size_t) context->m_Conn->GetAveLatency())
		{
...
int	App::handle_stream_messages( MsgHandlerContext *context )
{
	...
	switch( context->m_MsgId )
	{
		case MSG_ID_STREAM_START:
		{
			...
			if(((size_t) new_stream->m_DataPtr - (size_t) new_stream->m_Data ) >= (size_t) new_stream->m_Size )
			{
				MsgHandlerContext msg_context;
				int result;

				// Shouldn't go over
				Dbg_Assert( ((size_t) new_stream->m_DataPtr - (size_t) new_stream->m_Data ) == (size_t) new_stream->m_Size );
		...
		case MSG_ID_STREAM_DATA:
		{
					...
					str_link = str_sh.NextItem())
			{
				...
				if( desc->m_StreamId == msg->m_StreamId )
				{
					...
					if(((size_t) desc->m_DataPtr - (size_t) desc->m_Data ) >= (size_t) desc->m_Size )
					{
						MsgHandlerContext msg_context;
						int result;
						uint32 actual_crc;
						...
						// Shouldn't go over
						Dbg_Assert( ((size_t) desc->m_DataPtr - (size_t) desc->m_Data ) == (size_t) desc->m_Size );
