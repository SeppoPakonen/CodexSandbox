...
bool	App::init( void )
{
	...
	if( !IsLocal())
	{
		...
		if( setsockopt( m_socket, SOL_SOCKET, SO_NBIO, &true_val, sizeof( s32 )) == SOCKET_ERROR )
		{
			ReportError();
			return false;
		}
		...
		{
			if(( setsockopt( m_socket, SOL_SOCKET, SO_BROADCAST,
					(char*) &true_val, sizeof( s32 ))) == SOCKET_ERROR )
			{
				ReportError();
				return false;
			}
		...
		{
			if(( setsockopt( m_socket, SOL_SOCKET, SO_BROADCAST,
					(char*) &false_val, sizeof( s32 ))) == SOCKET_ERROR )
			{
				ReportError();
				return false;
			}
	...
	m_socket_thread_data.m_pEntry = (void*)&threaded_transfer_data;
		...
		if( conn->m_StreamOutList.CountItems() > 0 )
		{
			...
			if( num_pending < pending_threshold ) // Should be configurable and dynamic
			{
				...
				for( stream_link = stream_sh.FirstItem( conn->m_StreamOutList ); stream_link; stream_link = next_link )
				{
					...
					size = (size_t) stream_desc->m_DataPtr - (size_t) stream_desc->m_Data;
...
void	App::read_network_data( const Tsk::Task< App > &task )
{
	...
	app->m_Timestamp = (size_t) app->m_double_timestamp;
#else
...
#ifdef	__PLAT_NGPS__
	Dbg_MsgAssert(Mem::SameContext(this,MemManagerHandleNetworkHeap()),("Net::App not on network heap"));
#endif		//	__PLAT_NGPS__
}
...
void	App::ProcessData( void )
{
			...
			conn = next_conn )
	{
			...
			conn->m_alias_connection )
		{
			...
			num_bytes = (size_t) conn->m_alias_connection->m_write_ptr -
							(size_t) conn->m_alias_connection->m_write_buffer;
		...
		case QUEUE_IMPORTANT:
		{
					...
					msg_link = sh.NextItem())
			{
				...
				if( ( msg_link->m_Timestamp == 0 ) ||	// i.e. hasn't been sent yet
					(( m_Timestamp - msg_link->m_Timestamp ) > (size_t) conn->GetResendThreshold()))
				{
					int msg_header_size;
...
{
	if( (( m_Timestamp - conn->m_last_send_time ) >= (size_t) conn->GetSendInterval()) ||
