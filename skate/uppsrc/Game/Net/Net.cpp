...
static s32 set_host_name( sn_char* host_name )
{
    ...
    if (host_size > (s32) sizeof(optval.host))
    {
...
static s32 custom_construct_script( s32 isp_type, sn_char* user_name,
										 sn_char* password)
{
    s32  line_index;
    sn_bool   copy_done;
...
s32 custom_connect_modem(	sn_char*               phone_no,
								s32               isp_type,
								sn_char*               user_name,
								sn_char*               password,
								s32               timeout_secs,
...
void Manager::conn_modem_state_callback( s32 modem_state )
{
...
void Manager::disconn_modem_state_callback( s32 modem_state )
{
	Net::Manager * net_man = Net::Manager::Instance();
	...
	if(( IsOnline() == false ) || m_options_changed )
	{
				...
				( GetConnectionType() == vCONN_TYPE_PPPOE ))
		{
...

			{
				...
				m_modem_thread_data.m_pEntry = (void*)&threaded_modem_conn;
...
bool	Manager::DisconnectFromInternet( void )
{
			 ...
			 ( m_conn_type == vCONN_TYPE_PPPOE ))
	{
		...
		m_modem_thread_data.m_pEntry = (void*)&threaded_modem_disconn;
		m_modem_thread_data.m_iInitialPriority = vSOCKET_THREAD_PRIORITY;
		m_modem_thread_data.m_pStackBase = m_modem_thread_stack;
		m_modem_thread_data.m_iStackSize = vMODEM_THREAD_STACK_SIZE;
...
QueuedMsg::QueuedMsg( unsigned char msg_id, unsigned short msg_len, void* data )
{
	...
	m_MsgLength = msg_len;
#ifdef	__PLAT_NGPS__
	Dbg_MsgAssert(Mem::SameContext(this,MemManagerHandleNetworkHeap()),("QueuedMsg not on network heap"));
#endif		//	__PLAT_NGPS__
}
...
QueuedMsgSeq::QueuedMsgSeq( unsigned char msg_id, unsigned short msg_len, void* data, unsigned char group_id )
{
	...
	m_GroupId = group_id;
#ifdef	__PLAT_NGPS__
	Dbg_MsgAssert(Mem::SameContext(this,MemManagerHandleNetworkHeap()),("QueuedMsgSeq not on network heap"));
#endif		//	__PLAT_NGPS__
