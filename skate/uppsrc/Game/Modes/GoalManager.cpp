...
void CGoalManager::SetEndRunType( uint32 goalId, int newEndRunType )
{
	CGoal* pGoal = GetGoal( goalId );
	Dbg_Assert( pGoal );

	pGoal->SetEndRunType( (EndRunType)newEndRunType );
}
...
bool ScriptSetEndRunType( Script::CStruct* pParams, Script::CScript* pScript )
{
	...
	pGoalManager->SetEndRunType( (EndRunType)goalId, newEndRunType );
	return true;
}
