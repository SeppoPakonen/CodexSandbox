...
bool CSkaterProfile::Reset(Script::CStruct* pParams)
{
	...
	MemManagerHandlePushContext(MemManagerHandleSkaterInfoHeap());
	...
	m_Info.AddComponent( CRCD(0xd544aa2d,"trick_mapping"), ESYMBOLTYPE_STRUCTUREPOINTER, (size_t)pTemp );

	// as well as a special tricks array
	pTemp = new Script::CStruct;
	m_Info.AddComponent( CRCD(0xddbee809,"specials"), ESYMBOLTYPE_STRUCTUREPOINTER, (size_t)pTemp );
...
{
	MemManagerHandlePushContext(MemManagerHandleScriptHeap());
...
void CSkaterProfile::decompress_trick_mappings( const char* pTrickMappingName )
{
	...
	pTempStructure->AddComponent( CRCD(0x68a9b4e1,"compressed"), ESYMBOLTYPE_INTEGER, 0LL );
}
...
void CSkaterProfile::WriteIntoStructure( Script::CStruct* pStuff )
{
	...
	if (!IsPro())
	{
		...
		pSkaterInfo->AddComponent( CRCD(0x554c7d6f,"Appearance"), ESYMBOLTYPE_STRUCTUREPOINTER, (size_t)pTemp );
	}
	...
	if ( pFaceTexture && pFaceTexture->IsValid() )
	{
		...
		pSkaterInfo->AddComponent( CRCD(0xf0d3bc94,"FaceTexture"), ESYMBOLTYPE_STRUCTUREPOINTER, (size_t)pTemp );
	}

	pTemp=new Script::CStruct;
	pTemp->AppendStructure(&m_Info);
	pSkaterInfo->AddComponent( CRCD(0x3476cea8,"Info"), ESYMBOLTYPE_STRUCTUREPOINTER, (size_t)pTemp );
	...
	pSkaterInfo->AddComponent( CRCD(0x95388a60,"DeckFlags"), ESYMBOLTYPE_INTEGER, 0LL );

	pStuff->AddComponent(Name,ESYMBOLTYPE_STRUCTUREPOINTER,(size_t)pSkaterInfo);
}
