...
Obj::CGapChecklist	*		CSkaterCareer::GetGapChecklist(int level)
{
	...
	Dbg_MsgAssert(level >= 0 && level < (size_t)vMAX_CAREER_LEVELS,("bad level number (%d) in  GetGapChecklist",level));
...
void	CSkaterCareer::StartLevel(int level)
{
	...
	Dbg_MsgAssert(m_current_level < (size_t)vMAX_CAREER_LEVELS,( "level number %d too big",m_current_level));
...
{
	Dbg_MsgAssert( level_num >= 0 && level_num < (size_t)vMAX_CAREER_LEVELS, ( "MarkLevelVisited called with bad level num %i", level_num ) );
...
int CSkaterCareer::GetBestMedal(int Level)
{
	...
	Dbg_MsgAssert(Level>=0 && Level<(size_t)vMAX_CAREER_LEVELS,("Bad Level of %d sent to GetBestMedal",Level));
...
void CSkaterCareer::WriteIntoStructure(Script::CScriptStructure *pIn)
{
	...
	pTemp->AddComponent(Script::GenerateCRC("GoalFlags"),ESYMBOLTYPE_ARRAY,(size_t)pGoalFlags);
	pTemp->AddComponent(Script::GenerateCRC("LevelFlags"),ESYMBOLTYPE_ARRAY,(size_t)pLevelFlags);
	pTemp->AddComponent(Script::GenerateCRC("GlobalFlags"),ESYMBOLTYPE_ARRAY,(size_t)pGlobalFlags);

#if 1
	pTemp->AddComponent(Script::GenerateCRC("GapChecklists"),ESYMBOLTYPE_ARRAY,(size_t)pGapChecklists);
#endif
	...
	pIn->AddComponent(Script::GenerateCRC("Career"),ESYMBOLTYPE_STRUCTUREPOINTER,(size_t)pTemp);
...
void CSkaterCareer::ReadFromStructure(Script::CScriptStructure *pIn)
{
	...
	if ( pVisitedLevels )
	{
		...
		Dbg_MsgAssert( size <= (size_t)vMAX_CAREER_LEVELS, ( "wrong array size" ) );
#endif		// __NOPT_ASSERT__
		for ( int i = 0; i < (size_t)vMAX_CAREER_LEVELS; i++ )
