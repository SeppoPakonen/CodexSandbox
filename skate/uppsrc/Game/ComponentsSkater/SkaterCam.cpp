...
*****************************************************************************/
	...
	const float SC_VERT_AIR_LANDED_TIME	= 10.0f / 60.0f;
	const float SC_TILT_MAX				= 0.34907f;						// 20 degees.
	const float SC_TILT_MIN				= -0.34907f;					// -20 degees.
	const float SC_TILT_INCREMENT			= 2.0f * ( SC_TILT_MAX / 60.0f );	// 40 degrees/second.
	const float SC_TILT_DECREMENT			= 2.0f * ( SC_TILT_MAX / 60.0f );	// 40 degrees/second.
	const float SC_TILT_RESTORE			= 8.0f * ( SC_TILT_MAX / 60.0f );	// 160 degrees/second.
...
//			Gfx::AddDebugLine( mpSkater->m_pos, ( target[Z] * 72.0f ) + mpSkater->m_pos, MAKE_RGB( 0, 0, 128 ), MAKE_RGB( 0, 0, 128 ), 4 );
		...
		if( state == LIP )
		{
			mTiltAddition += SC_TILT_INCREMENT * Tmr::FrameRatio();
			if( mTiltAddition > SC_TILT_MAX )
			{
				mTiltAddition = SC_TILT_MAX;
			}
		}
		else if( mTiltAddition > 0.0f )
		{
			mTiltAddition -= SC_TILT_RESTORE * Tmr::FrameRatio();
...
//		    frame_matrix = t_matrix;
		...
		else
		{
			...
			else
			{
				...
				mVertAirLandedTimer = SC_VERT_AIR_LANDED_TIME;
			}
		}
