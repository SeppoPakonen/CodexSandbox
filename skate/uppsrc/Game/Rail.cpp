...
inline float span_4(float a, float b, float c, float d)
{
	float min = a;
	if (b < min) min = b;
	if (c < min) min = c;
...
inline float overlap(float a, float b, float c, float d)
{
	return  Mth::Abs(b-a) + Mth::Abs(d-c) - span_4(a,b,c,d);
}

inline bool nearly_the_same(float a, float b, float c, float d, float nearly = 1.0f)
{
