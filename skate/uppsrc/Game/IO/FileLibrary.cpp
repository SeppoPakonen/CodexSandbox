...
#define XFER_SIZE ( sizeof( SLibHeader ) + ( vMAX_LIB_FILES * sizeof( SFileInfo ) ) )
	...
	for ( int i = m_numFiles - 1; i >= 0; i-- )
	{
		...
		Dbg_MsgAssert( MemManagerHandleCutsceneTopDownHeap(), ( "No cutscene heap?" ) );
		...
		Dbg_MsgAssert( MemManagerHandleCutsceneBottomUpHeap(), ( "No cutscene heap?" ) );
		MemManagerHandlePushContext(MemManagerHandleCutsceneBottomUpHeap());
