...
//			if (size || (addr >(size_t) __text_objend))
//			{
//				*d++ = addr;		// store the address of the symbol
//				*d++ = (size_t)p;			// store the start of the symbol name
...
//			{
//				int size;
//	//			printf ("sp = %p, ra = %p %s\n",sp,ra,MemView_GetFunctionName((size_t)ra));
//				printf ("%p: %s\n",ra,MemView_GetFunctionName((size_t)ra,&size));
//			}
//		}
//
//		// test to see if we have recursed up all the way...
//		if (abs(int((size_t)ra - (size_t)&ENTRYPOINT)) < 1024
//		   || (size_t)ra &3
//		   || (size_t)ra < 0x100000
//		   || (size_t)ra > (size_t)_code_end		// and check it's not totally crazy....
//		   )
//		{
//			return 0;
//		}
...
void MemView_DumpBlockInfo(int cursor)
{
	...
	if (pBlock == NULL)
	{
//		if (cursor > (size_t)__text_org && cursor < (size_t)__bss_objend)		// check to see if in code/data
//		{
//
//			if (cursor < (size_t)__data_org)
//				printf("Code: ");
//			else if (cursor < (size_t)__rodata_org)
//				printf("Data: ");
//			else if (cursor < (size_t)__bss_org)
//				printf("RO-Data: ");
//			else
//				printf("BSS: ");
//
...
//		else
	...
	{
		void * p_start = (void*)((size_t)pBlock + Mem::Allocator::BlockHeader::sSize);
...
//
//	int i_cursor = (size_t)cursor;
//
//	Mem::Allocator::BlockHeader *pBlock = MemView_FindBlock(i_cursor);
//
//	if (pBlock)
//	{
//		blockstart = (size_t)((size_t)pBlock + Mem::Allocator::BlockHeader::sSize);
//		int size = pBlock->mSize;
//		blockend = (size_t)((size_t)blockstart + size);
//	}
//
//	if (start != last_start)
//	{
...
//	{
//		uint16 *source = (uint16*)((size_t)(f_source + f_off) &~1);
//
//		uint32 word;
...
//		else
//		{
//			if (blockstart && (size_t)(source)>=blockstart && (size_t)(source) <blockend)
//			{
//				word = *source;
//			   	word |= color;
//			}
...
//		if (!(x&3) && x > (size_t)_code_end /*&& x < (size_t)_std_mem_end*/) // don't check for end now, as we have some debug heaps up there we want to include
//		{
...
int MemView_CountBlocks(Mem::Allocator::BlockHeader *p_header)
{
	...
	while ( p_header )
	{
		void * p_start = (void*)((size_t)p_header + Mem::Allocator::BlockHeader::sSize);

		*p_used++ = (size_t)p_start;	 		// store the start of the block
...
//		num_used += MemView_CountBlocks(MemManagerHandleBottomUpHeap()->first_block());
//		num_used += MemView_CountBlocks(MemManagerHandleTopDownHeap()->first_block());
//		num_used += MemView_CountBlocks(MemManagerHandleFrontEndHeap()->first_block());
//		num_used += MemView_CountBlocks(MemManagerHandleNetworkHeap()->first_block());
//		num_used += MemView_CountBlocks(MemManagerHandleScriptHeap()->first_block());
//		num_used += MemView_CountBlocks(MemManagerHandleSkaterHeap(0)->first_block());
////		num_used += MemView_CountBlocks(MemManagerHandleDebugHeap()->first_block());
//		printf (" %d\n",num_used);
//		printf ("Sorting .....\n");
...
//		uint32 *stack_end = (uint32*)(((size_t)(stack_start) + (size_t)_stack_size-4) & ~(size_t)(_stack_size-1));
...
void MemView_DumpRefs(int addr)
{
	...
	while (1)
	{
		...
		TODO uint32 *p_start = 0;
		uint32 *p_end = 0;
		while (p_start<p_end && count < 10)
		{
			if (*p_start == (size_t)addr && p_start != (uint32*)&blockstart)
			{
				count++;
				printf ("\nReference level %d in %p\n",count,(void*)p_start);
				MemView_DumpBlockInfo((size_t)p_start);
				addr = (size_t)p_start;


				Mem::Allocator::BlockHeader *pBlock = MemView_FindBlock(addr);
				addr = (size_t)((size_t)pBlock + Mem::Allocator::BlockHeader::sSize);
				if (addr == (size_t) p_first || addr == last_addr)
				{
					printf ("LOOPING .....\n");
					return;
				}
...
void MemView_DumpFragments(Mem::Heap *pHeap)
{
	...
	while (p_full)
	{
		...
		{
			//printf ("\nFramgented Block\n\n");
			void * p_start = (void*)((size_t)p_full + Mem::Allocator::BlockHeader::sSize);
			MemView_DumpBlockInfo((size_t)p_start);
...
void MemView_DumpBottomFragments()
{
	...
	MemView_DumpFragments(MemManagerHandleBottomUpHeap());
}

void MemView_DumpTopFragments()
{

	MemView_DumpFragments(MemManagerHandleTopDownHeap());
}
...
////		if (Mem::SameContext(p_start,MemManagerHandleFrontEndHeap()))
//		{
