...
SHed *LoadHed( const char *filename )
{
	...
	uint32 *pHed = (uint32 *)new (MemManagerHandleTopDownHeap()) uint8[fileSize];
	...
	while ( p32[0] != 0xffffffff )
	{
		...
		len = ( len + 3 ) & ~3;
		p32 = (uint32*)(((size_t)&p32[2]) + len);
	}

	// Build list of checksums.
	uint32 *p_offset = new (MemManagerHandleTopDownHeap()) uint32[files];
	uint32 *p_size = new (MemManagerHandleTopDownHeap()) uint32[files];
	uint32 *dir_check = new (MemManagerHandleTopDownHeap()) uint32[files];
	uint32 *file_check = new (MemManagerHandleTopDownHeap()) uint32[files];
	int count = 0;
	p32 = pHed;
	while ( p32[0] != 0xffffffff )
	{
