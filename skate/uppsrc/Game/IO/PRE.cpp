...
PreFile::PreFile(uint8 *p_file_buffer, bool useBottomUpHeap)
{
	...
	for (int i = 0; i < m_numEntries; i++)
	{
		...
		{
			pFile = new (MemManagerHandleBottomUpHeap()) _File;
		}
		else
		{
			pFile = new (MemManagerHandleTopDownHeap()) _File;
		}
...
PreFile::FileHandle *PreFile::GetContainedFile(const char *pName)
{
	...
	if (!mp_activeFile->pData)
	{
		if (mp_activeFile->compressedDataSize)
		{
			...
			if (m_use_bottom_up_heap)
			{
				mp_activeFile->pData = new (MemManagerHandleBottomUpHeap()) uint8[mp_activeFile->m_filesize];
			}
			else
			{
				mp_activeFile->pData = new (MemManagerHandleTopDownHeap()) uint8[mp_activeFile->m_filesize];
			}
			Mem::PopMemProfile();
...
#	if !defined( __PLAT_NGC__ ) || ( defined( __PLAT_NGC__ ) && !defined( __NOPT_FINAL__ ) )
	...
	if (async)
	{
		...
		if (p_fileHandle)
		{
			...
			if (useBottomUpHeap)
			{
				pFile = new (MemManagerHandleBottomUpHeap()) uint8[file_size];
			}
			else
			{
				pFile = new (MemManagerHandleTopDownHeap()) uint8[file_size];
			}
			Mem::PopMemProfile();

			// Set the callback
			p_fileHandle->SetCallback(async_callback, (size_t) this, (size_t) pFile);
...
#	if !defined( __PLAT_NGC__ ) || ( defined( __PLAT_NGC__ ) && !defined( __NOPT_FINAL__ ) )
	...
	if (useBottomUpHeap)
	{
		if (!mp_table->PutItem(pFilename, new (MemManagerHandleBottomUpHeap()) PreFile(pFile,useBottomUpHeap)))
			Dbg_MsgAssert(0,( "PRE %s loaded twice", pFilename));
	}
	else
	{
		if (!mp_table->PutItem(pFilename, new (MemManagerHandleTopDownHeap()) PreFile(pFile)))
			Dbg_MsgAssert(0,( "PRE %s loaded twice", pFilename));
	}
}
...
void   	PreMgr::postLoadPre(CAsyncFileHandle *p_file_handle, uint8 *pData, int size)
{
	...
	for (int i = 0; i < m_num_pending_pre_files; i++)
	{
		if (m_pending_pre_files[i].mp_file_handle == p_file_handle)
		{
			// the PRE file object winds up at the top of the heap, too. This is fine because
			// it will be unloaded at the same time as the big file buffer
			if (!mp_table->PutItem(m_pending_pre_files[i].m_file_name, new (MemManagerHandleTopDownHeap()) PreFile(pData)))
			{
...
void PreMgr::LoadPre(const char *pFilename, bool async, bool dont_assert, bool useBottomUpHeap)
{
	...
	if (useBottomUpHeap)
	{
		...
		MemManagerHandlePushContext(MemManagerHandleBottomUpHeap());
	}
	else
	{
