...
s32 dvd_file_read_anywhere( DVDFileInfo* fileInfo, void* addr, s32 length, s32 offset )
{
	...
	if ( (((size_t)addr) & 0x0000001f) || (offset & 3) ) {
		// Mis-aligned.
//		printf ( "mis-aligned read %d bytes, offset %d\n", length, offset );
		bytes = (char *)(OSRoundUp32B(((size_t)byteData)));
		toread = length;
		p8 = (char *)addr;
		off = offset;
	...
	switch( access_type )
	{
		...
		case FOPEN:
		{
			...
			if ( DVDOpen( dvd_file_fix_name( name ), &p_file->m_FileInfo ) )
			{
				...
				p_file->mp_data = new (MemManagerHandleTopDownHeap()) char[p_file->m_size];
...
//	}
	...
	switch( access_type )
	{
		...
		case FGETS:
		{
			...
			return (size_t)dvd_file_gets( p_file, dest, num_chars );
		}
