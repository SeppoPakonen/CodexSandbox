...
bool CBonedAnimFrameData::plat_read_stream(uint8* pData, bool delete_buffer)
{
	...
	if ( m_flags & nxBONEDANIMFLAGS_OBJECTANIMDATA )
	{
		...
		pData += ( m_numBones * sizeof(size_t) );
	}
	else
	{
		mp_boneNames = NULL;
...
CBonedAnimFrameData::~CBonedAnimFrameData()
{
	...
	NsARAM::free( (size_t)mp_perBoneQFrameSize );
	NsARAM::free( (size_t)mp_qFrames );
	NsARAM::free( (size_t)mp_tFrames );
//	if ( ( m_flags & nxBONEDANIMFLAGS_PARTIALANIM ) && mp_partialAnimData )
//	{
//		NsARAM::free( (size_t)mp_partialAnimData );
//	}
#else
...
//		MemManagerHandlePopContext();
		...
		// Set the callback
		mp_fileHandle->SetCallback(async_callback, (size_t) this, (size_t) assertOnFail);
...
bool CBonedAnimFrameData::GetInterpolatedCameraFrames(Mth::Quat* pRotations, Mth::Vector* pTranslations, float time, Nx::CQuickAnim* pQuickAnim )
{
	...
	dma_count( (size_t)mp_perBoneFrames, size );
	while ( framecount_active );
//	if ( m_flags & nxBONEDANIMFLAGS_PARTIALANIM )
//	{
//		dma_partial( (size_t)mp_partialAnimData, ( m_numBones * sizeof( uint16 ) * 2 ) );
//	}
//	while ( framecount_active );

//	NsDMA::toMRAM( qqq, (size_t)mp_qFrames, m_num_qFrames * sizeof( CHiResAnimQKey ) );
//	NsDMA::toMRAM( ttt, (size_t)mp_tFrames, m_num_tFrames * sizeof( CHiResAnimTKey ) );
...
//	pCurrentTFrame = p_httt;
	...
	uint32 anim_q_offset = (size_t)mp_qFrames;
	uint32 anim_t_offset = (size_t)mp_tFrames;
...
#ifdef __ARAM__
	return (size_t)p_data - (size_t)pData;
#else
	return (char*)p_data;
#endif		// __ARAM__
}
...
bool CBonedAnimFrameData::GetCompressedInterpolatedFrames(Mth::Quat* pRotations, Mth::Vector* pTranslations, float time, Nx::CQuickAnim* pQuickAnim )
{
	...
	dma_count( (size_t)mp_perBoneQFrameSize, ( m_numBones * sizeof( uint16 ) * 2 ) + 16 );		// +16 should cover partial mask data.
//	if ( m_flags & nxBONEDANIMFLAGS_PARTIALANIM )
//	{
//		while ( framecount_active );
//		dma_partial( (size_t)mp_partialAnimData );
//	}
//	uint32 * partial = (uint32 *)&framecount[( (size_t)mp_partialAnimData - (size_t)mp_perBoneQFrameSize ) * 2];
	uint32 * partial = (uint32 *)&framecount[( (size_t)mp_partialAnimData - (size_t)mp_perBoneQFrameSize ) / 2];

	// Pre-DMA all data.
//	uint32 q_lines = ((size_t)mp_boneNames) >> 16;
//	uint32 t_lines = ((size_t)mp_boneNames) & 0xffff;
//	q_lines = ( ( q_lines + ( ARAM_CACHE_LINE_SIZE - 1 ) ) / ARAM_CACHE_LINE_SIZE );
//	t_lines = ( ( t_lines + ( ARAM_CACHE_LINE_SIZE - 1 ) ) / ARAM_CACHE_LINE_SIZE );

//	for ( uint32 lp = 0; lp < q_lines; lp++ ) dma_q_entry( ((size_t)mp_qFrames), lp );
//	for ( uint32 lp = 0; lp < t_lines; lp++ ) dma_t_entry( ((size_t)mp_tFrames), lp );
...
//			skip_this_bone = ( ( (*pBoneMask) & ( (1<<31) >> (i%32) ) ) == 0 );
		...
		if( !skip_this_bone )
		{
			...
			{
				// DMA this q track.
				uint aligned_off = ( (size_t)pNextQFrame & ~31 );
				uint size = ( ( *pQSizes - ( aligned_off - (size_t)pCurrentQFrame ) ) + 31 ) & ~31;
				DCFlushRange ( qqq, size );
				framecount_size = size;
				framecount_active = 1;
				ARQPostRequest	(	&framecount_request,
									...
									size,												// Length.
									arqCallback );										// Callback
				q_off = (size_t)pNextQFrame - aligned_off;
			}
#endif		// __ARAM__
...
#				endif		// __ARAM__
			...
			{
				// DMA this q track.
				int aligned_off = ( (size_t)pNextTFrame & ~31 );
				int size = ( ( *pTSizes - ( aligned_off - (size_t)pCurrentTFrame ) ) + 31 ) & ~31;
				DCFlushRange ( qqq, size );
				framecount_size = size;
				framecount_active = 1;
				ARQPostRequest	(	&framecount_request,
									...
									size,												// Length.
									arqCallback );										// Callback
				t_off = (size_t)pNextTFrame - aligned_off;
			}
#endif		// __ARAM__
...
//	NsDMA::toMRAM( p_sqqq, (size_t)mp_qFrames, m_num_qFrames * sizeof( CStandardAnimQKey ) );
//	NsDMA::toMRAM( p_sttt, (size_t)mp_tFrames, m_num_tFrames * sizeof( CStandardAnimTKey ) );
