 ...
 *  LOCAL VARIABLES
 ******************************************************************************/
static VidChunk mov_workChunk ATTRIBUTE_ALIGN(32);
static void Movies_dvdDoneCallback(s32 result, DVDFileInfo *videoInfo);
 ...
 ******************************************************************************
 */
static void Movies_ReadFrameAsync(void)
	{
	if (!player.error && player.preFetchState == TRUE)
		...
		if (DVDReadAsync(&player.fileHandle,
						 ...
						 (s32)player.nextFrameOffset, Movies_dvdDoneCallback) != TRUE )
			{
 ...
 *		the next frame, which is used in Movies_ReadFrameAsync().
 *
 *
 * \note
 ...
 ******************************************************************************
 */
static void Movies_dvdDoneCallback(s32 result, DVDFileInfo * _UNUSED(videoInfo))
	{
...
BOOL VIDSimpleOpen(char* fileName, BOOL suppressAudio)
    ...
    if (DVDRead(&player.fileHandle, &mov_workChunk, 32, 0) < 0)
		{
#ifdef _DEBUG
	...
	// Check file id
	if(mov_workChunk.id != VID_FCC('V','I','D','1')  )
		{
#ifdef _DEBUG
	...
	if(mov_workChunk.vid.versionMajor != 1 || mov_workChunk.vid.versionMinor != 0)
		{
#ifdef _DEBUG
		OSReport("*** Unsupported file version: major: %d, minor: %d\n",
				  mov_workChunk.vid.versionMajor, mov_workChunk.vid.versionMajor);
#endif
		DVDClose(&player.fileHandle);
		return FALSE;
		}
	...
	{
	u32 version = VID_VERSION(mov_workChunk.vid.vidConvMajor, mov_workChunk.vid.vidConvMinor, mov_workChunk.vid.vidConvBuild);
	if(version < VID_VERSION(1,6,4))
		OSReport("*** WARNING: Vid file created using an unsupported converter version: %d.%d.%d\n", (u32)mov_workChunk.vid.vidConvMajor, (u32)mov_workChunk.vid.vidConvMinor, (u32)mov_workChunk.vid.vidConvBuild);
	}
#endif

	// Check types of chunks we have in this file.
	// !!! Note that we assume start of 'HEAD' chunk at byte offset 32 from file start !!!
	if (DVDRead(&player.fileHandle, &mov_workChunk, 32, (s32)fileOffset) < 0)
		{
#ifdef _DEBUG
	...
	if(mov_workChunk.id != VID_FCC('H','E','A','D')  )
		{
#ifdef _DEBUG
	...
	player.nextFrameOffset = mov_workChunk.len + 32;
	...
	headSize = mov_workChunk.len - VID_CHUNK_HEADER_SIZE;
	while(headSize >= 32)
		{
		if (DVDRead(&player.fileHandle, &mov_workChunk, 32, (s32)fileOffset) < 0)
			{
		...
		// Video header?
		if(mov_workChunk.id == VID_FCC('V','I','D','H') )
			{
			// check if we have an old vid file.
			if(mov_workChunk.version == 0)
				{
				mov_workChunk.vidh.frameRateScale = (u16)(*((u32*)&mov_workChunk.vidh.frameRateScale));
				mov_workChunk.vidh.flags = 0;
				}

			// Yes...
			ASSERT(mov_workChunk.len <= 32);
			ASSERT(mov_workChunk.len <= (sizeof(VidVIDH) + VID_CHUNK_HEADER_SIZE));
			memcpy(&player.videoInfo, &mov_workChunk.vidh, sizeof(VidVIDH));
			}
		// It's an audio header chunk! May we initialize it?
		else if(mov_workChunk.id == VID_FCC('A','U','D','H') && !suppressAudio)
			{
			// Allocate memory for audio header chunk
            player.audioHeaderChunk = (u8*)((*player.cbAlloc)(mov_workChunk.len));
			audioInfoSize = mov_workChunk.len - VID_CHUNK_HEADER_SIZE;

			// Copy the already loaded part
			memcpy(player.audioHeaderChunk, &mov_workChunk, 32);
			mov_workChunk.len -= 32;

			// Read additional audio header bytes if the audio header is greater that 32 bytes
			if(mov_workChunk.len >= 32)
				{
				ASSERT((mov_workChunk.len&31)==0);
				if (DVDRead(&player.fileHandle, player.audioHeaderChunk+32, mov_workChunk.len, (s32)fileOffset) < 0)
					{
#ifdef _DEBUG
					OSReport("*** Error reading file at offset %d\n", fileOffset);
#endif
				...
				fileOffset += mov_workChunk.len;
				headSize -= mov_workChunk.len;
				}
		...
		// Skip unknown chunks. We already read 32 bytes for the header which we must subtract here.
			...
			{
			fileOffset += mov_workChunk.len - 32;
			headSize -= mov_workChunk.len - 32;
			}
        }
	...
	if (DVDRead(&player.fileHandle, &mov_workChunk, 32 , (s32)player.nextFrameOffset) < 0 )
		{
#ifdef _DEBUG
	...
	if(mov_workChunk.id != VID_FCC('F','R','A','M')  )
		{
#ifdef _DEBUG
	...
	player.nextFrameSize = mov_workChunk.len; 		// 32 bytes of this chunk are already consumed, but
...
{
	MemManagerHandlePushContext( MemManagerHandleTopDownHeap());
	MemManagerHandleBottomUpHeap()->PushAlign( 32 );
	void * p = new u8[size];
	MemManagerHandleBottomUpHeap()->PopAlign();
	MemManagerHandlePopContext();
	return p;
}
...
static void				*movies_audioReadBuffer;							// Buffer to store audio data received from the data stream
static u32				movies_audioReadBufferNumSamples;					// Size of the above buffer in samples
static u32				movies_audioReadBufferWritePos;					// Write position in the above buffer in samples
static u32				movies_audioReadBufferReadPos;						// Read position in the above buffer in samples
static u8				movies_audioReadBufferNumChannels;					// Number of channels stored in the read buffer

static void				*movies_audioPlayBuffer[VID_AUDIO_NUMAIBUFFERS];	// AI playback buffers
static u8				movies_audioPlayBufferWriteIndex;					// Index to next AI buffer to be written to from the read buffer
static u32				movies_audioPlayBufferFrq;							// Playback frequency of AI in Hz
static volatile BOOL	movies_audioPlayBufferEnabled;						// TRUE if playback is enabled. AI will operate at all times, but zeros will be filled in instead of real data if this is FALSE.
...
static const u32		*movies_audioPlayMaskArray;						// Pointer to an array of channel play masks (each set bit signals an active channel)
static u32				movies_audioNumPlayMasks;							// Number of play masks specified (0 indicates all channels should be active)
static u32				movies_audioNumActiveVoices;						// Number of active voices

static AXVPB			*movies_axVoice[2];								// AX voice structures
static ARQRequest		movies_arqRequest[2][VID_AUDIO_NUMAIREQUESTS];		// Enough ARQ request structures for worst case scenario
static u32				movies_axLastAddr;									// Last known address DSP read from for 1st voice
static u32				movies_axPlayedSamples;							// Number of samples played on first voice since last update
...
typedef enum MOV_VID_AXPHASE {
		MOV_AX_PHASE_STARTUP = 0,
		MOV_AX_PHASE_START,
		MOV_AX_PHASE_PLAY
} MOV_VID_AXPHASE;

static MOV_VID_AXPHASE		movies_axPhase;

/******************************************************************************
 *  LOCAL PROTOTYPES
 ******************************************************************************/

static void Movies_AXCallback(void);
...
BOOL VIDSimpleInitAudioDecoder(void)
	...
	if (player.audioInfo.audioID != VID_FCC('N','O','N','E'))
		...
		if (player.audioInfo.audioID == VID_FCC('V','A','U','D'))
			...
			movies_audioReadBufferNumSamples = (u32)(((f32)VID_AUDIO_READAHEADFRAMES * player.audioInfo.vaud.frq) / player.fps);
			movies_audioReadBufferNumChannels = player.audioInfo.vaud.numChannels <= 2 ? player.audioInfo.vaud.numChannels : 2;
	        ...
	        if (player.audioInfo.audioID == VID_FCC('P','C','1','6'))
				{
		        // Calculate buffer size to allocate proper memry to keep a bit of "extra" audio data around...
		        movies_audioReadBufferNumSamples = (u32)(((f32)VID_AUDIO_READAHEADFRAMES * player.audioInfo.pcm16.frq) / player.fps);

		        if (player.audioInfo.pcm16.numChannels <= 2)
			        movies_audioReadBufferNumChannels = player.audioInfo.pcm16.numChannels;
		        else
			        movies_audioReadBufferNumChannels = 2;
		        ...
		        if (player.audioInfo.audioID == VID_FCC('A','P','C','M'))
			        ...
			        // (the idea being to decode the data into the movies_audioReadBuffer allocated below)
			        // [...]
			        ASSERT(FALSE);
			        }
		        }
		...
		movies_audioReadBuffer = player.cbAlloc(movies_audioReadBufferNumSamples * sizeof(s16) * player.audioInfo.pcm16.numChannels);
		if (movies_audioReadBuffer == NULL)
			return TRUE;					// error

		// Reset ring buffer
		movies_audioReadBufferReadPos = movies_audioReadBufferWritePos = 0;

		// What AI frquency is best?
		movies_audioPlayBufferFrq = player.audioInfo.pcm16.frq;

		// Allocate AI playback buffer
		movies_audioPlayBuffer[0] = player.cbAlloc(2 * sizeof(s16) * VID_AUDIO_AIBUFFERSAMPLES * VID_AUDIO_NUMAIBUFFERS);
		if (movies_audioPlayBuffer[0] == NULL)
			return TRUE;					// error

		for(i=1; i<VID_AUDIO_NUMAIBUFFERS; i++)
			movies_audioPlayBuffer[i] = (void *)((size_t)movies_audioPlayBuffer[i - 1] + (2 * sizeof(s16) * VID_AUDIO_AIBUFFERSAMPLES));

		// Reset buffer index
		movies_audioPlayBufferWriteIndex = 0;

		// We disable AI output for now (logically)
		movies_audioPlayBufferEnabled = FALSE;
		audioPlayBackPossible = FALSE;

		// We assume to playback all we get by default
		movies_audioPlayMaskArray = NULL;
		movies_audioNumPlayMasks = 0;
		movies_audioNumActiveVoices = 2;

		// Clear out AI buffers to avoid any noise what so ever
		memset(movies_audioPlayBuffer[0],0,2 * sizeof(s16) * VID_AUDIO_AIBUFFERSAMPLES * VID_AUDIO_NUMAIBUFFERS);
		DCFlushRange(movies_audioPlayBuffer[0],2 * sizeof(s16) * VID_AUDIO_AIBUFFERSAMPLES * VID_AUDIO_NUMAIBUFFERS);

		// Init GCN audio system
		old = OSDisableInterrupts();

		movies_axVoice[0] = AXAcquireVoice(AX_PRIORITY_NODROP,NULL,0);
		ASSERT(movies_axVoice[0] != NULL);
		movies_axVoice[1] = AXAcquireVoice(AX_PRIORITY_NODROP,NULL,0);
		ASSERT(movies_axVoice[1] != NULL);
		...
		AXSetVoiceMix(movies_axVoice[0],&axMix[0]);
		AXSetVoiceMix(movies_axVoice[1],&axMix[1]);

		axVE.currentDelta = 0;
		axVE.currentVolume = 0x7FFF;
		AXSetVoiceVe(movies_axVoice[0],&axVE);
		AXSetVoiceVe(movies_axVoice[1],&axVE);

		memset(&axSRC,0,sizeof(AXPBSRC));

		ratio = (u32)(65536.0f * (f32)movies_audioPlayBufferFrq / (f32)AX_IN_SAMPLES_PER_SEC);
		axSRC.ratioHi = (u16)(ratio >> 16);
		axSRC.ratioLo = (u16)ratio;

		axCurrentFrq = (f32)movies_audioPlayBufferFrq;

		// Calculate what we deem is a save amount of extra data in our read buffers
		axMinAvailFrames = (u32)((movies_audioPlayBufferFrq * (VID_AUDIO_LEADFACTOR - 1.0f) * ((f32)VID_AUDIO_NUMLEADFRAMES / player.fps)) / (f32)VID_AUDIO_AIBUFFERSAMPLES);

		AXSetVoiceSrcType(movies_axVoice[0],AX_SRC_TYPE_4TAP_16K);
		AXSetVoiceSrc(movies_axVoice[0],&axSRC);
		AXSetVoiceSrcType(movies_axVoice[1],AX_SRC_TYPE_4TAP_16K);
		AXSetVoiceSrc(movies_axVoice[1],&axSRC);
		...
		*(u32 *)&axAddr.endAddressHi = AX_ARAM_LEFT_CHANNEL + AX_ARAM_BUFFER_SIZE - 1;
		...
		AXSetVoiceAddr(movies_axVoice[0],&axAddr);
		...
		*(u32 *)&axAddr.endAddressHi = AX_ARAM_RIGHT_CHANNEL + AX_ARAM_BUFFER_SIZE - 1;
		AXSetVoiceAddr(movies_axVoice[1],&axAddr);

		memset(&axADPCM,0,sizeof(axADPCM));
		axADPCM.gain = 0x0800;

		AXSetVoiceAdpcm(movies_axVoice[0],&axADPCM);
		AXSetVoiceAdpcm(movies_axVoice[1],&axADPCM);

		AXSetVoiceType(movies_axVoice[0],AX_PB_TYPE_STREAM);
		AXSetVoiceType(movies_axVoice[1],AX_PB_TYPE_STREAM);

		AXRegisterCallback(Movies_AXCallback);

		movies_axLastAddr = AX_ARAM_LEFT_CHANNEL;
		movies_axPlayedSamples = VID_AUDIO_AIBUFFERSAMPLES * VID_AUDIO_NUMAIBUFFERS;
		movies_axPhase = MOV_AX_PHASE_STARTUP;
...
void VIDSimpleExitAudioDecoder(void)
	...
	if (player.audioInfo.audioID != VID_FCC('N','O','N','E'))
		...
		AXSetVoiceState(movies_axVoice[0],AX_PB_STATE_STOP);
		AXSetVoiceState(movies_axVoice[1],AX_PB_STATE_STOP);
		AXFreeVoice(movies_axVoice[0]);
		AXFreeVoice(movies_axVoice[1]);

		movies_axVoice[0] = movies_axVoice[1] = NULL;
		...
		player.cbFree(movies_audioPlayBuffer[0]);
		player.cbFree(movies_audioReadBuffer);
		}
	}
...
void VIDSimpleAudioReset(void)
	...
	if (player.audioInfo.audioID != VID_FCC('N','O','N','E'))
		...
		movies_audioReadBufferWritePos = 0;
		movies_audioReadBufferReadPos = 0;
...
static void VIDSimpleDoAudioDecodePCM16(s16* dest, u32 channels, const s16** samples, u32 sampleOffset, u32 sampleNum)
	...
	if (movies_audioNumPlayMasks != 0)
		...
		u32 c = 0;
		u32 rc = 0;
		for(i=0; i<movies_audioNumPlayMasks; i++)
			{
			for(b=0; b<32; b++)
				{
				if (movies_audioPlayMaskArray[i] & (1<<b))
					{
					for(s=0; s<numSamples; s++)
						dest[s * movies_audioReadBufferNumChannels + c] = ((s16 *)src)[s * player.audioInfo.pcm16.numChannels + rc];
					c++;

					// If we already updated all read buffer channels we may exit. There's no more space anyways...
					if (c == movies_audioReadBufferNumChannels)
						return;
					}
				rc++;
 ...
 ******************************************************************************
 */
static BOOL Movies_audioDecode(u32 numChannels, const s16 **samples, u32 sampleNum, void* userData)
	{
	u32		freeSamples;
	u32		sampleSize;
	u32		len1;
	...
	sampleSize = sizeof(s16) * movies_audioReadBufferNumChannels;

	// How many samples could we put into the buffer?
	if (movies_audioReadBufferWritePos >= movies_audioReadBufferReadPos)
		{
		freeSamples = movies_audioReadBufferNumSamples - (movies_audioReadBufferWritePos - movies_audioReadBufferReadPos);

		if (freeSamples < sampleNum)
			{
			OSRestoreInterrupts(old);
			#ifndef FINAL
			OSReport("*** Movies_audioDecode: overflow\n");
			#endif
			return FALSE;				// overflow!
			}

		// We might have a two buffer update to do. Check for it...
		if ((len1 = (movies_audioReadBufferNumSamples - movies_audioReadBufferWritePos)) >= sampleNum)
			{
			// No. We got ourselfs a nice, simple single buffer update.
			VIDSimpleDoAudioDecode((s16 *)((size_t)movies_audioReadBuffer + movies_audioReadBufferWritePos * sampleSize),numChannels,samples,0,sampleNum);
			}
		else
			{
			// Dual buffer case
			VIDSimpleDoAudioDecode((s16 *)((size_t)movies_audioReadBuffer + movies_audioReadBufferWritePos * sampleSize),numChannels,samples,0,len1);
			VIDSimpleDoAudioDecode((s16 *)movies_audioReadBuffer,numChannels,samples,len1,sampleNum-len1);
			}
		}
	else
		{
		freeSamples = movies_audioReadBufferReadPos - movies_audioReadBufferWritePos;
	...
	movies_audioReadBufferWritePos += sampleNum;

	if (movies_audioReadBufferWritePos >= movies_audioReadBufferNumSamples)
		movies_audioReadBufferWritePos -= movies_audioReadBufferNumSamples;
...
BOOL VIDSimpleAudioDecode(const u8* bitstream, u32 bitstreamLen)
	...
	if (player.audioInfo.audioID != VID_FCC('N','O','N','E'))
        ...
        if(player.audioInfo.audioID == VID_FCC('V','A','U','D'))
			...
			if(!VIDAudioDecode(player.decoder, bitstream+headerSize, bitstreamLen-headerSize, 0x3, Movies_audioDecode, NULL))
				return FALSE;
			...
			if(!Movies_audioDecode(1, (const s16**)&data, VIDSimpleAudioSamplesFromBytes(dataLength), NULL))
				return FALSE;
...
void VIDSimpleAudioStartPlayback(const u32 *playMaskArray,u32 numMasks)
	...
	if (audioPlayBackPossible)
		{
		movies_audioPlayBufferEnabled = TRUE;
...
void VIDSimpleAudioChangePlayback(const u32 *playMaskArray,u32 numMasks)
	...
	movies_audioPlayMaskArray = playMaskArray;
	movies_audioNumPlayMasks = numMasks;

	// Any playback mask specified?
	if (movies_audioNumPlayMasks != 0)
		{
		// Yes. Count the active voices...
		movies_audioNumActiveVoices = 0;
		...
		for(i=0; i<numMasks; i++)
			...
			for(b=0; b<32; b++)
				{
				if (movies_audioPlayMaskArray[i] & (1<<b))
					movies_audioNumActiveVoices++;
				}
			}

		// So, did we have too much?
		ASSERT(movies_audioNumActiveVoices <= 2);
		}
	else
		movies_audioNumActiveVoices = movies_audioReadBufferNumChannels;		// Make all active...
 ...
 ******************************************************************************
 */
static void Movies_audioCopy(u32 smpOffset,u32 numSamples)
	{
	s16		*destAddrL,*destAddrR,*srcAddr,s;
	u32		j;

	// Target address in audio buffer
	destAddrL = (s16 *)((size_t)movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex] + smpOffset * sizeof(s16));
	destAddrR = (s16 *)((size_t)movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex] + smpOffset * sizeof(s16) + (VID_AUDIO_AIBUFFERSAMPLES * sizeof(s16)));

	// Single or dual channel setup?
    if (movies_audioReadBufferNumChannels == 2)
		{
		// Stereo! Get source address of data...
		srcAddr = (s16 *)((size_t)movies_audioReadBuffer + movies_audioReadBufferReadPos * 2 * sizeof(s16));
		...
		ASSERT(movies_audioReadBufferNumChannels == 1);

		// Get source address...
		srcAddr = (s16 *)((size_t)movies_audioReadBuffer + movies_audioReadBufferReadPos * sizeof(s16));
	...
	movies_audioReadBufferReadPos += numSamples;
	if (movies_audioReadBufferReadPos >= movies_audioReadBufferNumSamples)
		movies_audioReadBufferReadPos -= movies_audioReadBufferNumSamples;
	}
...
static void Movies_AXCallback(void)
	...
	u32		movies_audioPlayBufferNeededAudioFrames;
	u32		currentAddr;
	...
	if (movies_axPhase == MOV_AX_PHASE_START)
	{
		AXSetVoiceState(movies_axVoice[0],AX_PB_STATE_RUN);
		AXSetVoiceState(movies_axVoice[1],AX_PB_STATE_RUN);
		movies_axPhase = MOV_AX_PHASE_PLAY;
	}
	else if( movies_axPhase == MOV_AX_PHASE_PLAY )
	{
		if( movies_axVoice[0]->pb.state == AX_PB_STATE_STOP )
		{
			return;
		}
	}
	...
	currentAddr = *(u32 *)&movies_axVoice[0]->pb.addr.currentAddressHi;

	if (currentAddr >= movies_axLastAddr)
		movies_axPlayedSamples += currentAddr - movies_axLastAddr;
	else
		movies_axPlayedSamples += ((AX_ARAM_LEFT_CHANNEL + AX_ARAM_BUFFER_SIZE) - movies_axLastAddr) + (currentAddr - AX_ARAM_LEFT_CHANNEL);

	movies_axLastAddr = currentAddr;

	//OSReport("%d\n",movies_axPlayedSamples);

	if (movies_axPlayedSamples >= VID_AUDIO_AIBUFFERSAMPLES)
	{
		movies_audioPlayBufferNeededAudioFrames = movies_axPlayedSamples / VID_AUDIO_AIBUFFERSAMPLES;

		// Make sure that we never get an underrun we don't notice...
		if(!(movies_audioPlayBufferNeededAudioFrames <= VID_AUDIO_NUMAIBUFFERS))
		{
//			OSReport("AX audio buffer underrun!\n");
			movies_audioPlayBufferEnabled = false;
		}

		//ASSERT(movies_audioPlayBufferNeededAudioFrames <= VID_AUDIO_NUMAIBUFFERS);

		// Is actual audio playback enabled?
		if (movies_audioPlayBufferEnabled)
		{
			// How many samples could we get from the read buffer?
			if (movies_audioReadBufferWritePos >= movies_audioReadBufferReadPos)
				availSamples = movies_audioReadBufferWritePos - movies_audioReadBufferReadPos;
			else
				availSamples = movies_audioReadBufferNumSamples - (movies_audioReadBufferReadPos - movies_audioReadBufferWritePos);

			// That's how many audio frames?
			availFrames = availSamples / VID_AUDIO_AIBUFFERSAMPLES;

			// Are the voice already playing?
			if (movies_axPhase == MOV_AX_PHASE_PLAY)
			{
				f32 targetFrq;

				// Yes. We better watch out for the buffer status, so we don't get under-runs...
				if (availFrames < axMinAvailFrames)
					targetFrq = movies_audioPlayBufferFrq * (1.0f - (1.0f/200.0f) * (axMinAvailFrames - availFrames));
				else
					targetFrq = (f32)movies_audioPlayBufferFrq;

				// Track the target frequency quite slowly to avoid audible artifacts
                if (axCurrentFrq < targetFrq)
				{
					axCurrentFrq += movies_audioPlayBufferFrq * 0.00125f;
					if (axCurrentFrq > targetFrq)
				...
				// Set frequency
				AXSetVoiceSrcRatio(movies_axVoice[0],axCurrentFrq / (f32)AX_IN_SAMPLES_PER_SEC);
				AXSetVoiceSrcRatio(movies_axVoice[1],axCurrentFrq / (f32)AX_IN_SAMPLES_PER_SEC);
			}

			//OSReport("AX: %d %d (%d)\n",availSamples,movies_audioPlayBufferNeededAudioFrames * VID_AUDIO_AIBUFFERSAMPLES,movies_axPlayedSamples);

			// So, how many can we update?
			numUpdate = (availFrames > movies_audioPlayBufferNeededAudioFrames) ? movies_audioPlayBufferNeededAudioFrames : availFrames;

			// If anything... go do it!
			if (numUpdate != 0)
			{
				movies_axPlayedSamples -= numUpdate * VID_AUDIO_AIBUFFERSAMPLES;
				...
				for(i=0; i<numUpdate; i++)
				{
					...
					if ((n = movies_audioReadBufferNumSamples - movies_audioReadBufferReadPos) < VID_AUDIO_AIBUFFERSAMPLES)
					{
						// It wraps...
						Movies_audioCopy(0,n);
						Movies_audioCopy(n,VID_AUDIO_AIBUFFERSAMPLES - n);
					}
					else
					{
						// We got one continous source buffer
						Movies_audioCopy(0,VID_AUDIO_AIBUFFERSAMPLES);
					}

					// Make sure the data ends up in real physical memory
					DCFlushRange(movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex],VID_AUDIO_AIBUFFERSAMPLES * sizeof(s16) * 2);

					leftSource = (size_t)movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex];
					rightSource = (size_t)movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex] + (VID_AUDIO_AIBUFFERSAMPLES * sizeof(s16));
					leftTarget = 2 * (AX_ARAM_LEFT_CHANNEL + movies_audioPlayBufferWriteIndex * VID_AUDIO_AIBUFFERSAMPLES);
					rightTarget = 2 * (AX_ARAM_RIGHT_CHANNEL + movies_audioPlayBufferWriteIndex * VID_AUDIO_AIBUFFERSAMPLES);

					// Make sure we get this into ARAM ASAP...
					ARQPostRequest(&movies_arqRequest[0][i%VID_AUDIO_NUMAIREQUESTS],0,ARQ_TYPE_MRAM_TO_ARAM,ARQ_PRIORITY_HIGH,leftSource,leftTarget,VID_AUDIO_AIBUFFERSAMPLES * sizeof(s16),NULL);
					ARQPostRequest(&movies_arqRequest[1][i%VID_AUDIO_NUMAIREQUESTS],1,ARQ_TYPE_MRAM_TO_ARAM,ARQ_PRIORITY_HIGH,rightSource,rightTarget,VID_AUDIO_AIBUFFERSAMPLES * sizeof(s16),NULL);

					// Advance write index...
					movies_audioPlayBufferWriteIndex = (u8)((movies_audioPlayBufferWriteIndex + 1) % VID_AUDIO_NUMAIBUFFERS);
				}

				if (movies_axPhase == MOV_AX_PHASE_STARTUP)
					movies_axPhase = MOV_AX_PHASE_START;
			}
		}
		else
		{
			// Update buffer(s) with silence...
			movies_axPlayedSamples -= movies_audioPlayBufferNeededAudioFrames * VID_AUDIO_AIBUFFERSAMPLES;

			for(i=0; i<movies_audioPlayBufferNeededAudioFrames; i++)
			{
				memset(movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex],0,2 * sizeof(s16) * VID_AUDIO_AIBUFFERSAMPLES);
				DCFlushRange(movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex],2 * sizeof(s16) * VID_AUDIO_AIBUFFERSAMPLES);

				leftSource = (size_t)movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex];
				rightSource = (size_t)movies_audioPlayBuffer[movies_audioPlayBufferWriteIndex] + (VID_AUDIO_AIBUFFERSAMPLES * sizeof(s16)) / 2;
				leftTarget = 2 * (AX_ARAM_LEFT_CHANNEL + movies_audioPlayBufferWriteIndex * VID_AUDIO_AIBUFFERSAMPLES);
				rightTarget = 2 * (AX_ARAM_RIGHT_CHANNEL + movies_audioPlayBufferWriteIndex * VID_AUDIO_AIBUFFERSAMPLES);
