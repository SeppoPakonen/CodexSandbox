...
struct Rect_ : Moveable<Rect_<T>> {
	...
	Size_<T> GetSize() const { return Size_<T>(Width(), Height()); }

	bool Contains(const Point_<T>& pt) {
...
public:
	...
	static bool Process(const Vector<Pointf>& contour,
		Vector<Trianglef>& result);
