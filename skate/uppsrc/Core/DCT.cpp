...
void ConvertToParamsLine(int point_count, const Point* pt, int size, char* params) {
	...
	double len = FastSqrt(diff.x * diff.x + diff.y * diff.y);
	int steps = max((int)(max(diff.x, diff.y) / 2), 1);
	
	pts.SetCount(0);
	
	double line_angle = FastAtan2(diff.y, diff.x);
	...
	while (it != end) {
		if (it == begin) {
			...
			Point diff = *it - *begin;
			double len = FastSqrt(diff.x * diff.x + diff.y * diff.y);
			double pt_angle = FastAtan2(diff.y, diff.x);
...
void ConvertFromParamsLineAppend(int param_count, const char* params, Point begin, Point end, Vector<Point>& pts) {
	...
	it.x = begin.x;
	it.y = begin.y;
	double shift_angle = FastAtan2(diff.y, diff.x) - M_PI / 2.0;
...
void ForwardDCT1d8AAN(const double* i, double* o) {
	...
	const double s0 = (FastCos(0) * FastSqrt(.5) / 2) / (1);   // 0.353553
