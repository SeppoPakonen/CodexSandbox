...
inline void ActiveTexture(TexArb texture) {glActiveTextureARB(texture);}
...
inline void DepthRange(Float near, Float far) {glDepthRangef(near, far);}

inline void MultiTexCoord(TexArb target, Gxf s, Gxf t, Gxf r, Gxf q) {glMultiTexCoord4fARB(target, s, t, r, q);}

NAMESPACE_GFXSTATIC_END
