...
u8 DOOpenGLOptimizeOn = 1;
#define OPENGL_STATE_NOT_OPTIMIZED 0
#define OPENGL_STATE_OPTIMIZING 1
#define OPENGL_STATE_OPTIMIZED 2

typedef f32 MtxGL[16];

/*---------------------------------------------------------------------*
Name:			MTXtoGL

Description:	converts a matrix (Mtx format) to a gl matrix (MTXGL format) 

Arguments:		Mtx			input matrix
				MTXGL*		output matrix

Return:			nothing.
*---------------------------------------------------------------------*/
static void MTXtoGL( Mtx input, MtxGL* glmatrix )
{
	(*glmatrix)[0] = input[0][0];
	(*glmatrix)[1] = input[1][0];
	(*glmatrix)[2] = input[2][0];
	(*glmatrix)[3] = 0.0f;

	(*glmatrix)[4] = input[0][1];
	(*glmatrix)[5] = input[1][1];
	(*glmatrix)[6] = input[2][1];
	(*glmatrix)[7] = 0.0f;

	(*glmatrix)[8] = input[0][2];
	(*glmatrix)[9] = input[1][2];
	(*glmatrix)[10] = input[2][2];
	(*glmatrix)[11] = 0.0f;

	(*glmatrix)[12] = input[0][3];
	(*glmatrix)[13] = input[1][3];
	(*glmatrix)[14] = input[2][3];
	(*glmatrix)[15] = 1.0f;
...
void    DOVARender ( DODisplayObjPtr dispObj, Mtx camera, u8 numLights, va_list list )
{
...
#if defined(flagWIN32) && defined(OPENGL_OPTIMIZE)
    // optimize only if we are not stitching
    if(SkinForwardArray == 0) {
		// Check to see if object has been optimized for OpenGL
		// If so, Draw it as an OpenGL display list
		if (dispObj->displayInit == OPENGL_STATE_OPTIMIZED && DOOpenGLOptimizeOn) 
		{
			MTXtoGL(mv, &glMtx);
			glMatrixMode( GL_MODELVIEW );
			glPushMatrix();
			glLoadMatrixf(glMtx);
			glCallList(dispObj->displayList);
			glPopMatrix();
			// Done!
			return;
		}
		// Otherwise, create an openGL display list for this object
		// Note: There are two passes (indicated by displayInit)
		// The first pass allows the Textures to get loaded into 
		// cache. On the second pass, the textures are loaded, so
		// the display list can be created.
		else if (dispObj->displayInit == OPENGL_STATE_OPTIMIZING && DOOpenGLOptimizeOn)
		{
			// Draw at Origin
			MTXIdentity(identMtx);
			GXLoadPosMtxImm(identMtx, GX_PNMTX0);
			GXSetCurrentMtx(GX_PNMTX0);
	
			// Create Display List
			dispObj->displayList = glGenLists(1);
			glNewList(dispObj->displayList, GL_COMPILE);
		}
    }
#endif
    ...
    // optimize only if we are not stitching
    if(SkinForwardArray == 0) {
		if (dispObj->displayInit == OPENGL_STATE_NOT_OPTIMIZED && DOOpenGLOptimizeOn)
		{
			// Prepare for next state
			dispObj->displayInit = OPENGL_STATE_OPTIMIZING;
		}
		else if (dispObj->displayInit == OPENGL_STATE_OPTIMIZING && DOOpenGLOptimizeOn)
		{
			// End Display List (Second Pass)
			glEndList();
			// The next state will be optimized!
			dispObj->displayInit = OPENGL_STATE_OPTIMIZED;
		}
	}
#endif
}
