		 ...
		 !HasFSpCreateScriptSupportFix() )
	{
		...
		if ( result == noErr )
		{
			...
			if ( result == noErr )
			{
				...
				//											((char)((char)scriptTag | (char)0x80)) :
				//											(smRoman);

				//Ryan did this to remove compiler warning!
				if(scriptTag >= smRoman)
					pb.ciPB.hFileInfo.ioFlXFndrInfo.fdScript = (char)((char)scriptTag | (char)0x80);
				else
					pb.ciPB.hFileInfo.ioFlXFndrInfo.fdScript = smRoman;
								   ...
								   StringPtr uniqueName)
{
	...
	for ( i = 0; i < 16; ++i )
	{
		if ( i < 10 )
		{
			hexStr[i] = (unsigned char)(0x30 + i);
		}
		else
		{
			hexStr[i] = (unsigned char)(0x37 + i);
		}
	}
	...
	while ( error != fnfErr )
	{
		...
		for ( i = 1; i <= 2; i++ )
		{
			error = (short)(error & PBGetCatInfoSync(&cinfo));
									   ...
									   const FSSpec *dest)
{
		 ...
		 !HasFSpExchangeFilesCompatibilityFix() )
	{
		...
		if ( result != noErr )
		{
			...
			temp = (long)(catInfoSource.hFileInfo.ioFlMdDat);
			catInfoSource.hFileInfo.ioFlMdDat = catInfoDest.hFileInfo.ioFlMdDat;
			catInfoDest.hFileInfo.ioFlMdDat = (unsigned long)temp;
		 ...
		 !HasFSpCreateScriptSupportFix() )
	{
		...
		if ( ResError() == noErr )
		{
			...
			if ( result == noErr )
			{
				...
				// Added by Ryan to remove compiler warnings
				if(scriptTag >= smRoman)
					pb.hFileInfo.ioFlXFndrInfo.fdScript = (char)((char)scriptTag | (char)0x80);
				else
					pb.hFileInfo.ioFlXFndrInfo.fdScript = smRoman;
				/* Set creator/fileType */
