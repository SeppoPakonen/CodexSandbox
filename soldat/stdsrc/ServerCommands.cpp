...
namespace ServerCommandsImpl {
    ...
    inline void CommandLoadList(const std::vector<std::string>& Args, uint8_t Sender) {
        ...
        if (Args.empty()) {
            ...
            Name = sv_maplist.Value();
            size_t dotPos = Name.find(".txt");
    ...
    inline void CommandDelMap(const std::vector<std::string>& Args, uint8_t Sender) {
        ...
        for (TempInt = 0; TempInt < MapsList.Count(); TempInt++) {
            std::string upperMapName = Name;
            std::transform(upperMapName.begin(), upperMapName.end(), upperMapName.begin(), ::toupper);
            std::string upperListName = MapsList[TempInt];
            std::transform(upperListName.begin(), upperListName.end(), upperListName.begin(), ::toupper);
    ...
    inline void CommandNetStats(const std::vector<std::string>& Args, uint8_t Sender) {
        // char StatsText[2049];  // 0-indexed array of 2049 elements (0-2048)
        // 
        // if (UDP.NetworkingSocket.GetDetailedConnectionStatus(1, StatsText, 2048) == 0) {
        //     WriteLn(StatsText);
        // }
        // 
        // for (int i = 1; i <= MAX_PLAYERS; i++) {
        //     if (Sprite[i].Active && Sprite[i].Player.Peer > 0) {
        //         if (UDP.NetworkingSocket.GetDetailedConnectionStatus(Sprite[i].Player.Peer, StatsText, 2048) == 0) {
        //             std::cout << "----------------" << std::endl;
        //             std::cout << Sprite[i].Player.Name << std::endl;
        //             std::cout << Sprite[i].Player.Peer << std::endl;
        //             std::cout << StatsText << std::endl;
        //             std::cout << "----------------" << std::endl;
        //         }
        //     }
        // }
    ...
    inline void CommandPlayerCommand(const std::vector<std::string>& Args, uint8_t Sender) {
        std::string Command;
        TVector2 a = {0.0f, 0.0f};
        ...
        if (Command == "tabac") {
            ...
            // Sprite[Sender].HealthHit(150, Sender, 1, -1, a); // Assuming this method exists
            if (Sprite[Sender].Player.Kills > 0) {
                Sprite[Sender].Player.Kills--;
            }
        }
            ...
            Sprite[Sender].Vest = 0;
            // Sprite[Sender].HealthHit(3423, Sender, 1, -1, a); // Assuming this method exists
            if (Sprite[Sender].Player.Kills > 0) {
                Sprite[Sender].Player.Kills--;
            }
        }
    ...
    inline void CommandInfo(const std::vector<std::string>& Args, uint8_t Sender) {
        ...
        // ServerSendStringMessage(L"Nextmap: " + WideString(CheckNextMap), Sender, 255, MSGTYPE_PUB);
    ...
    inline void InitServerCommands() {
        ...
        CommandAdd("admip", CommandAdmIP, "add the IP number to the admins list", {CMD_FLAG::CMD_ADMINONLY});
...
using ServerCommandsImpl::CommandNetStats;
using ServerCommandsImpl::CommandPlayerCommand;
...
// Global variables
TGun Guns[TOTAL_WEAPONS + 1];
TGun DefaultGuns[TOTAL_WEAPONS + 1];
