...
void CK_AnimateMapTeleporter(int tileX, int tileY)
{
	...
	for (CK_object *current_obj = ck_keenObj->next; current_obj != NULL; current_obj = current_obj->next)
	{

		if (current_obj->active || current_obj->type != 8 ||
			current_obj->clipRects.tileX2 < (rf_scrollXUnit >> 8) - 1 || current_obj->clipRects.tileX1 > (rf_scrollXUnit >> 8) + (320 >> 4) + 1 || current_obj->clipRects.tileY2 < (rf_scrollYUnit >> 8) - 1 || current_obj->clipRects.tileY1 > (rf_scrollYUnit >> 8) + (208 >> 4) + 1)
			continue;

		current_obj->visible = 1;
		current_obj->active = OBJ_ACTIVE;
		RF_AddSpriteDraw(&current_obj->sde, current_obj->posX, current_obj->posY, current_obj->gfxChunk, 0, current_obj->zLayer);
	}
